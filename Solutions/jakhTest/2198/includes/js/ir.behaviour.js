...
function pagination(data) {
    ...
    var amountOfNewsPerPage = 2;

    var pageNumber = 1;
    var selectedCategoryArr = [];
    for (var k = 0; k < allData.length; k++) {
        if ($('#' + allData[k].storyId).hasClass('category_' + currentID)) {
            selectedCategoryArr.push(allData[k]);
        }
    }
    ...
    for (var i = 0; i < selectedCategoryArr.length; i++) {
        ...
        if ($('#' + selectedCategoryArr[i].storyId).css('display') == 'none') {
            $('#' + selectedCategoryArr[i].storyId).removeClass('page' + pageNumber)
        }

        else if (i >= amountOfNewsPerPage) {
            $('#' + selectedCategoryArr[i].storyId).addClass('page' + pageNumber + ' hide');
        }

        else {
            $('#' + selectedCategoryArr[i].storyId).addClass('page' + pageNumber);
        }
    }
}
