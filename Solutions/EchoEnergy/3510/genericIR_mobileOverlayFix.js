...
function retrieveAndPostValues() {
    ...
    // Post data
    //
    var emailAlertsManagerURL = '//ir.euroinvestor.com/Tools/EmailAlertsWithPush/EmailAlertsManager.aspx?ts=' + (new Date).getTime(); // Accessible from asp/ir/*
    if (localhost) {
...
//        var isRSS = false;
        ...
        if (isActiveEmailAdress && !emailIsValid($("#enterInformationEmail").val())) {
            isOK = false;
            
            $("#enterInformationEmail").on('keyup', function(event) {
            //Check with email regex if input is valid then isOk = true
            if(event.target.id === 'enterInformationEmail') {
              if(emailIsValid(event.target.value)) {  
                $('.emailNotification').css('visibility', 'hidden');
                isOK = true;
              } else {
                  $('.emailNotification').css('visibility', 'visible');
                isOK = false;
              }
            }
        });
