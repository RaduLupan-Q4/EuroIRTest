        ...
        <tbody>
            <thead>
                <tr>
                    ...
                    <th class="Header price">{{headers/t_price}} (p)</th>
                    <th class="Header volume">{{headers/t_volume}}</th>
                    <th class="Header value column-last">{{headers/t_value}} (GBP)</th>
            ...
            <tr>
                ...
                <td class="Data timestampFull">{{showDateWithFormat timestamp 'DD MMM HH:mm'}}</td>
                <td class="Data price">{{decimals tradePrice}}</td>
                <td class="Data volume">{{toLocal tradeVolume}}</td>
                <td class="Data value column-last">{{showTradeValue tradePrice tradeVolume}}</td>
...
<script type="text/javascript">
    ...
    function checkLastPrice() {
        var lastTradePrice = parseFloat($('.IRTradesModule tr td:nth-child(2)')[0].innerText);
        var lastPrice = globalRawStockData[globalActiveListingIndex].last;
        var count = 1;
        if (lastTradePrice != formatDecimal(lastPrice)) {
            if (count == 1) {
                $('.IRTradesModule tr:first-child td').hide();
                count++;
            }
        } else {
            count = 1;
