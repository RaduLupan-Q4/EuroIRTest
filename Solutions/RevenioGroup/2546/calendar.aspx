...
<div class="IRCalendar">
    <div class="wrapper2">
        ...
        <div class="calendarRemindAllEncapsulator" id="Encapsulator2">
            ...
            <br />--%>
            	<div id="SubscribePrompt" class="privacy_statement_check">
                    <input type="checkbox" id="chksubscribe" onclick="enableSubscription()">
                    <span id="privacy_statement_check_text"></span>
                </div>
                    ...
                    <div class="headlineWrapper">
                        <div class="calendarDate">
                            <div class="publishDateWrapper">
                                <div class="date">{{showEventDate keyValueSet}}</div>
                                <div class="calendarTime">{{showEventTime keyValueSet 'HH:mm'}}</div>
...
<script type="text/javascript">
    ...
    function convertTimezone(timestamp) {
        ...
        return moment.utc(timestamp, "YYYY-MM-DD HH:mm:ss").local().format( navigator.appVersion.indexOf("MSIE 10") !== -1 || navigator.appVersion.indexOf("MSIE 9") !== -1 || navigator.appVersion.indexOf("Trident") !== -1 ? "YYYY-MM-DDTHH:mm:ssZ" : "YYYY/MM/DD HH:mm:ss");        
    }
    ...
    //Show End Date For Diffirent monts/days
    Handlebars.registerHelper('showEventDate', function (objects, format) {
        ...
        var startDate = getObjects([objects], 'key', 'eventstarttime');
        var formattedStartYearMonth = moment(new Date(startDate)).format('MMM DD');
        var endDate = getObjects([objects], 'key', 'eventendtime');
        var formattedEndYearMonth = moment(new Date(endDate)).format('- MMM DD, YYYY');
    
        
        var startMonthToCheck = moment(new Date(startDate)).format("YYYY MM");
        var endMonthToCheck = moment(new Date(endDate)).format("YYYY MM");
        var startDayToCheck = moment(new Date(startDate)).format("YYYY MM DD");
        var endDayToCheck = moment(new Date(endDate)).format("YYYY MM DD");
        var startYearToCheck = moment(new Date(startDate)).format("YYYY");
        var endYearToCheck = moment(new Date(startDate)).format("YYYY");
        
        
        if (endDate.length == 0) {
            return moment(new Date(startDate)).format('MMM DD, YYYY')
        }
        if (startMonthToCheck == endMonthToCheck && startDayToCheck != endDayToCheck) {
            return moment(new Date(startDate)).format('MMM DD') + moment(new Date(endDate)).format(' - DD, YYYY');   
        }
        if (startMonthToCheck !== endMonthToCheck) {
            return moment(new Date(startDate)).format('MMM DD') + moment(new Date(endDate)).format(' - MMM DD, YYYY'); 
        }
        if (startMonthToCheck == endMonthToCheck && startDayToCheck == endDayToCheck) {
            return moment(new Date(startDate)).format('MMM DD') + moment(new Date(endDate)).format(', YYYY'); 
        }
        if (startYearToCheck !== endYearToCheck) {
            return moment(new Date(startDate)).format('MMM DD, YYYY') + moment(new Date(endDate)).format('MMM DD, YYYY'); 
        }
            return ;
    });

    
    Handlebars.registerHelper('showEventTime', function (objects, format) {
        ...
        var eventstart = getObjects([objects], 'key', 'eventstarttime');
        var eventendtime = getObjects([objects], 'key', 'eventendtime');
        var formattedStartDate = moment(new Date(eventstart)).format(format);
        var formattedEndDate = moment(new Date(eventendtime)).format(format);
        
        var CETeventstart = getObjects([objects], 'key', 'eventstarttime');
        ...
        if ((formattedStartDate === "00:00" && formattedEndDate === "00:00") || (formattedStartDate === "00:00") || (moment.tz(new Date(CETeventstart), "Europe/Copenhagen").format("HH:mm") === "00:00") ) {
            return;
    ...
    //check privacy
    $('#privacy_statement_check_text').click(function () {
        if ($(this).hasClass('check')) {
            $('#privacy_statement_check_text').removeClass('check');
            $('#privacy_statement_check_text').removeClass('check');
            $("#chksubscribe").prop("checked", false);
        }
        else {
            $('#privacy_statement_check_text').addClass('check');
            $('#privacy_statement_check_text').addClass('check');
    ...
    function prepareCustomX() {
        if (!customXApplied) {
                        ...
                        $.when(requestTranslationsData)
                            .done(function (TranslationsData) {
                                ...
                                if (globalActiveLanguage == "da") {
                                    ...
                                    ////$('.t_signUpEmailReminders').html("Tilmeld Email PÃ¥mindelser");
                                    //$('.t_signUpEmailText').text("VÃ¦lg begivenheder ovenfor, som du Ã¸nsker at modtage email pÃ¥mindelser pÃ¥")
                                    ////$('.calendarReminderAll').text("Tilmeld alle begivenheder");
                                    //$('.sendReminders1').text("Send mig pÃ¥mindelser");
                                    //$('.sendReminders2').text("fÃ¸r begivenheden starter");
                                    //$('.sendRemindersTo').text("Send kalender begivenheder til");
                                    //$('#emailCalendarSignup_submit').text("Tilmeld");
                                    //$('.t_unsubscribe_text').html("Hvis du Ã¸nsker at afmelde, venligst indtast email og tryk pÃ¥ <b>afmeld</b>");
                                    $('.privacy_statement span').html("TÃ¤tÃ¤ palvelua yllÃ¤pitÃ¤Ã¤ Q4 Euroinvestor Revenio Group Oyj:n puolesta. Kunnioitamme yksityisyyttÃ¤ emmekÃ¤ myy, vuokraa tai jaa kenenkÃ¤Ã¤n yhteystietoja. Kaikki antamasi tiedot kÃ¤sitellÃ¤Ã¤n luottamuksellisina eikÃ¤ niitÃ¤ kytketÃ¤ kÃ¤ytetÃ¤ mihinkÃ¤Ã¤n mihin et ole suostunut.");
                                    ...
                                    $('.privacy_statement span').html("This service is operated by Q4 Euroinvestor on behalf of Revenio Group Corporation. We respect your privacy and do not sell, rent or loan any identifiable information collected on this site. Any information that you give us will be treated with the utmost care and security. It will not be used in ways to which you have not consented.");


                                }

                                if (globalActiveLanguage == "fi") {

                                    $('.privacy_statement span').html("TÃ¤tÃ¤ palvelua yllÃ¤pitÃ¤Ã¤ Q4 Euroinvestor Revenio Group Oyj:n puolesta. Kunnioitamme yksityisyyttÃ¤ emmekÃ¤ myy, vuokraa tai jaa kenenkÃ¤Ã¤n yhteystietoja. Kaikki antamasi tiedot kÃ¤sitellÃ¤Ã¤n luottamuksellisina eikÃ¤ niitÃ¤ kytketÃ¤ kÃ¤ytetÃ¤ mihinkÃ¤Ã¤n mihin et ole suostunut.");
                                    $('.privacy_statement .title').text(translationData.data.t_privacy_statement);
                                    $('#privacy_statement_check_text').text("Olen lukenut tietosuojatiedot ja tarjoavat lupaani.");


                                } else {
                                    $('.privacy_statement span').html("This service is operated by Q4 Euroinvestor on behalf of Revenio Group Corporation. We respect your privacy and do not sell, rent or loan any identifiable information collected on this site. Any information that you give us will be treated with the utmost care and security. It will not be used in ways to which you have not consented.");
                                    $('.privacy_statement .title').text(translationData.data.t_privacy_statement);
                                    $('#privacy_statement_check_text').text("I have read the privacy statement and provide my consent.");
            ...
            $("#emailCalendarSignup_submit").click(function () {
               ...
               if (isValidEmailAddress($('#emailCalendarSignup').val()) && checkedValues.length != 0 && ($('#chksubscribe').attr('checked'))) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (globalActiveLanguage == "da") {
                                ...
                                $('.information').text("Du har succesfuldt tilmeldt email pÃ¥mindelser.");
                    ...
                    if (globalActiveLanguage == "da") {
                        $('.verificationImage').attr('src', 'images/information.png');
                        $('.informationHeader').text("Ups, noget mangler!");
                        $('.information').text("SÃ¦t venligst hak i nogle events.");
                        $('#dialogueBox_register').show();
                        $('.transparantOverlay').show();
                    } else {
                        $('.verificationImage').attr('src', 'images/information.png');
                    ...
                    if (globalActiveLanguage == "da") {
                        $('.verificationImage').attr('src', 'images/information.png');
                        $('.informationHeader').text("Ups, noget mangler!");
                        $('.information').text("Venligst sÃ¦t hak i nogle events og indtast gyldig email.");
                        $('#dialogueBox_register').show();
                        $('.transparantOverlay').show();
                    } else {
                        $('.verificationImage').attr('src', 'images/information.png');
                ...
                if (isValidEmailAddress($('#emailCalendarSignup').val()) && (!$('#chksubscribe:checked').length)) {
                    if (globalActiveLanguage == "fi") {
                        $('.verificationImage').attr('src', 'images/information.png');
                        $('.informationHeader').text("Oho, jotain puuttuu!");
                        $('.information').text("Ottakaa vastaan, ettÃƒÂ¤ hyvÃƒÂ¤ksyt varastointiin tietosi varten tarjota sinulle ilmoituksia.");
                        $('#dialogueBox_register').show();
                        $('.transparantOverlay').show();
                    } else {
                        $('.verificationImage').attr('src', 'images/information.png');
                        $('.informationHeader').text("Oops, something is missing!");
                        $('.information').text("Please accept that you agree to the storage of your data for the purpose of providing you with the notifications.");
                        $('#dialogueBox_register').show();
                        $('.transparantOverlay').show();
                    }
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (globalActiveLanguage == "da") {
                                ...
                                $('.information').text("Du har succesfuldt afmeldt email pÃ¥mindelser.");
            ...
            $(".calendarDetails").click(function () {
                ...
                //undgÃ¥ at siden scroller til tops
                return false;
            });
