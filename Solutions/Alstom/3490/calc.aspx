...
<script id="IRCalcTemplate" type="text/x-handlebars-template">
    <div class="IRChartCalcPlaceholder">

    </div>
    
	<span class="x-curency">EUR</span>
	
    <form id="calc-form">
        <div class="input-row">
            <label for="from-datepicker" class="input-label from-label" style="float: left;">{{t_period}}:</label>
        ...
        <div class="input-row">
            <label for="to-datepicker" class="input-label to-label"> </label>
            <div class="input-wrapper">
                {{{datepicker 'to'}}}
                {{{selectToDay}}}
                {{{selectToMonth}}}
        ...
        <div class="input-row">
            ...
            <div class="input-wrapper">
                <span id="currency-symbol">EUR: </span>
...
function addRadioBtn(){
    ...
    $(document).on('change', "input[type=radio]", function(){
	    ...
	    function thous(){
        var mass = [ '.buy-price-cell', ".sell-price-cell", ".value-now-cell", '.est-shares-cell'];

        for (var i = 0; i < mass.length; i++){
            var temp = $(mass[i]).first().text().replace(/[^0-9.,]/g, '');
            if(temp.length != 0){
                var txt = $(mass[i]).text().replace(temp, "")
                var nb = temp.toString().split(clientLocaleParameters.decimalSeparator);
                if (nb[1] !== 'undifined') $(mass[i]).text(nb[0].replace(clientLocaleParameters.decimalSeparator1000, '').replace(/\B(?=(\d{3})+(?!\d))/g, clientLocaleParameters.decimalSeparator1000) + clientLocaleParameters.decimalSeparator + nb[1]);
                else $(mass[i]).text(nb[0].replace(clientLocaleParameters.decimalSeparator1000, '').replace(/\B(?=(\d{3})+(?!\d))/g, clientLocaleParameters.decimalSeparator1000) + clientLocaleParameters.decimalSeparator);
            } else {
                 $(mass[i]).text('-');
            }
        }
