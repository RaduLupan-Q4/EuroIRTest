...
$(function () {
    ...
    language = url.split('?').pop().split('=').pop();
});
function init() {
    $('.pageRegister').show();
...
function checkInputFields() {
    ...
    if (activePage == 1) {
        ...
        //    }
        //}
        if ($('.checkboxOuter input[type=checkbox]:checked').length === 0) {
            isOK = false;
        }
    ...
    if (!isOK) {
        if (language == 'se') {
            $('.requiredFieldMSG').html('Vänligen, fyll i alla obligatoriska fält');
    ...
    if (data.msgToReturn != "") {
        ...
        if (language == 'se') {
            if (data.msgToReturn == "InvalidInput") {
                ...
                html = '<a class="cleanLink goToHome">Tillbaka</a>';
            }
            ...
            if (data.msgToReturn == "UserEmailExistAndIsNotActivedRNSFilter") {
                ...
                html = '<p class="bold">Please click on the link provided in the email message to verify your information.</p><a class="cleanLink goToHome">Tillbaka</a>';
            }
            ...
            if (data.msgToReturn == "UserCreated") {
                header = 'Fantastiskt! Du är nästan klar!';
                paragraph = 'Vänligen, gå till din inbox för att bekräfta registrering av e-postmeddelanden';
                html = '';
            }
            if (data.msgToReturn == "UserDoesNotExist") {
                header = 'Verifikation';
                paragraph = 'Ogiltig email adress.';
                html = '<a class="cleanLink goToHome">Tillbaka</a>';
            }
            if (data.msgToReturn == "UserDoesNotExist") {
                header = 'Verifikation';
            ...
            if (data.msgToReturn == "UserUnsubscribedWasNotActive") {
                header = 'Verifikation';
                paragraph = 'Din prenumeration är nu avslutad';
                html = '<a class="cleanLink goToHome">Tillbaka</a>';
                $('.formContinue.unsubscribeInTool').hide();
            }
            ...
            if (data.msgToReturn == "InvalidInput") {
                header = 'Verification';
                paragraph = 'Email entered is not valid. Please try again.';
                html = '<a class="cleanLink goToHome">X</a>';
            }
            if (data.msgToReturn == "UserEmailExistAndIsNotActived") {
                header = 'Verification';
                paragraph = 'Email already subscribed but not yet activated.';
            ...
            if (data.msgToReturn == "UserCreated") {
                header = 'Fantastic, you are almost there!';
                paragraph = 'Please check your inbox to confirm your email alert subscription.';
                html = '';
            }
            if (data.msgToReturn == "UserDoesNotExist") {
                header = 'Verification';
