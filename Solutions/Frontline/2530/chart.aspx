...
<div class="IRChartToolMenu IRChartChangeListing"></div><br />
    ...
    <div class="companyInfoWrapper">
        <div class="chartName">{{stocks/name}} (<%--{{stocks/exchangeName}}--%>{{stocks/exchangeName}}: {{stocks/symbol}})</div>
        <div class="updated"><%--<span>{{headers/t_updated}}: </span>--%><span>{{showTime stocks/timestamp}} {{showLocalTimeZoneShort}} on {{showDateWithFormat stocks/timestamp 'DD MMM, YYYY'}}</span></div>
    </div>
    ...
    <table class="IRQuoteModule table-look horizontal customResponsive">
        ...
        <tr>
            <td class="Data symbol column-first" rowspan="3" style="vertical-align: middle" id="chartLast"><span style="font-size: 30px;">{{stocks/currency}}</span> {{decimals stocks/last}}</td>
