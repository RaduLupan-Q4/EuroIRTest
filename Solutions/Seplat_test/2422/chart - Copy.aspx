...
<script id="IRQuoteTableTemplate" type="text/x-handlebars-template">
    <table class="IRToolQuoteTable table-look table-look-horizontal quote quote-horizontal responsive-horizontal">
        ...
        {{/headers}}
            {{#stocks}}
<tr><th>Symbol</th><th>Open</th><th>Prev Close</th><th>Day High</th><th>Day Low</th></tr>
<tr><td>{{symbol}}</td><td>{{decimalsNoZero open}}</td><td>{{decimals prevClose}}</td><td>{{decimalsNoZero high}}</td><td>{{decimalsNoZero low}}</td></tr>
<tr><th>Price</th><th>Change</th><th>Change (%)</th><th>Volume</th><th>Mid</th></tr>
<tr><td>{{decimals last}}</td><td>{{decimals change}}</td><td>{{decimals changePercent}}</td><td>{{toLocal volume}}</td><td>{{calculatedMid bid ask}}</td></tr>
<tr><th>Bid</th><th>Ask</th><th>52 Week-high</th><th>52 Week-low</th><th>Trade time</th></tr>
<tr><td>{{decimalsNoZero bid}}</td><td>{{decimalsNoZero ask}}</td><td>{{decimals high52Week}}</td><td>{{decimals low52Week}}</td><td>{{showDateTime tradeTimestamp}}</td></tr>
