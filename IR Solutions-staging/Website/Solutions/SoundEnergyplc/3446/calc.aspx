...
function addRadioBtn(){
	...
	function thous(){
        var mass = [ '.buy-price-cell', ".sell-price-cell", ".value-now-cell"];
        for (var i = 0; i < mass.length; i++){
            var temp = $(mass[i]).first().text().replace(/[^0-9]+/, '');
            if(temp.length != 0){
                var txt = $(mass[i]).first().text().replace(temp, "")
                var nb = temp.toString().split(".");
                $(mass[i]).text(txt+nb[0].replace(/\B(?=(\d{3})+(?!\d))/g, clientLocaleParameters.decimalSeparator1000) + clientLocaleParameters.decimalSeparator + nb[1]);
            } else {
                 $(mass[i]).text('-');
            }
        }
    ...
    function shares(){
        var mass = [ '.est-shares-cell'];
        for (var i = 0; i < mass.length; i++){
            var temp = $(mass[i]).first().text().replace(/^[^\d]+/, '');
            if(temp.length != 0){
                var txt = $(mass[i]).text().replace(temp, "")
                var nb = temp.toString().split(".");
                $(mass[i]).text(nb[0].replace(/\B(?=(\d{3})+(?!\d))/g, clientLocaleParameters.decimalSeparator1000) + clientLocaleParameters.decimalSeparator + nb[1]);
            } else {
                 $(mass[i]).text('-');
            }
        }
    ...
    $(function () {
        window.setInterval(shares, 200);
    })

    function yield(){
    var changeEl = ['.yield-cell']
        for (var i = 0; i < changeEl.length; i++){
            ...
            else $(changeEl[i]).text(" "+nb[0].replace(/\B(?=(\d{3})+(?!\d))/g, clientLocaleParameters.decimalSeparator1000) + clientLocaleParameters.decimalSeparator + nb[1] + txt);
            }
        }
    }

	$(function () {
        window.setInterval(yield, 200);
    })
</script>
