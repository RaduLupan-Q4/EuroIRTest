...
function calculateOrdersTableSum() {
    var bid = 0;
    if (typeof $(".bidSize") != "undefined" && typeof $(".totalVolumeBid") != "undefined") {
        $(".bidSize").each(function () {
            var value = $(this).data('bidsize');
            if (!isNaN(value) && value.length != 0) bid += parseFloat(value);
        });
        $(".totalVolumeBid").text(NumberFormat.decimal1000(bid));
    }
    if (typeof $(".askSize") != "undefined" && typeof $(".totalVolumeAsk") != "undefined") {
        var ask = 0;
        $(".askSize").each(function () {
            var value = $(this).data('asksize');
            if (!isNaN(value) && value.length != 0) ask += parseFloat(value);
        });
        $(".totalVolumeAsk").text(NumberFormat.decimal1000(ask));
    }
}
...
function buildCalcTool(data, template) {
    ...
    IRCalcModuleBuild.initializeCalc(data);
...
function buildLookupTool(data, template) {
    ...
    if (typeof ($(ModulesList.IRLookupModule.view).html()) != "undefined" && typeof ($(ModulesList.IRLookupModule.template).html()) != "undefined") {
        ...
        IRLookupModuleBuild.initializeLookup(data);
        ModulesReady.endModule(ModulesList.IRLookupModule.name);
    }
}
function buildCustomTool(data) {
    debugStep("buildIRChartTool");
...
function generatePagination(place) {
    ...
    if (clientStyle.paginationShowNumber > 0) {
        paginationHTML += '<div class="IRNewsPaginationPageNew IRNewsPaginationPageFirst inactive"> << </div>';
        paginationHTML += '<div class="IRNewsPaginationPageNew IRNewsPaginationPagePrev inactive"> < </div>';

        for (i = 1; i <= globalNewsPagesInTotal; i++) {
            if (i <= clientStyle.paginationShowNumber) paginationHTML += '<div class=\"IRNewsPageNumber IRNewsPageNumber' + i + '" data-page=\"' + i + '\">' + i + '</div>';
            else paginationHTML += '<div class=\"IRNewsPageNumber hiddenPage IRNewsPageNumber' + i + '" data-page=\"' + i + '\">' + i + '</div>';
        }

        paginationHTML += '<div class="IRNewsPaginationPageNew IRNewsPaginationPageNext"> > </div>';
        paginationHTML += '<div class="IRNewsPaginationPageNew IRNewsPaginationPageLast"> >> </div>';
    } else {
        for (i = 1; i <= globalNewsPagesInTotal; i++) {
...
function drawActiveListingToChartHistorical() {
    ...
    if (typeof (globalChartDom) != 'undefined') {
        if (FeaturesList.IRChartSettings.use && globalDefaultSettings.activeSetType === 'dot') {
        ...
        } else if (FeaturesList.IRChartSettings.use && globalDefaultSettings.activeSetType === 'candlestick') {
            globalChartDom.addSeries({
                index: 1,
...
function drawPlotLineToChart() {
    ...
    var text = "";
    text += '<div class="chartCurrentPriceBoxOuter"><div class="chartCurrentPriceBox" style="border: 1px solid ' + clientStyle.chart_ColourMain + ';background-color: ' + clientStyle.chart_ColourMain + ';">' +
        '<div class="chartCurrentPriceBoxArrow">' +
        '<div class="irCPB1" style="background-color: ' + clientStyle.chart_ColourMain + ';"></div>' +
        '<div class="irCPB2" style="background-color: ' + clientStyle.chart_ColourMain + ';"></div>' +
        '<div class="irCPB3" style="background-color: ' + clientStyle.chart_ColourMain + ';"></div>' +
        '</div>';
    text += '<div class="chartLastPrice">' + NumberFormat.decimal1000Zeros(lastPrice) + '</div><br />';
...
// Calc & Lookupchart
function drawIRChartCalcLookup(data) {
    debugStep("populateChart");
    var transformedData = stockDataToChartData(data, 'date', 'closePrice');
    var defaultChart = $.extend(true, {}, mainChart);
    defaultChart.chart.borderWidth = 0;
    defaultChart.chart.marginRight = clientStyle.lookup_ChartYAxisInsideOutside === 'inside' ? 5 : 50;
    defaultChart.chart.marginLeft = 5;
    defaultChart.chart.spacingTop = 5;
    defaultChart.chart.spacingBottom = 5;
    defaultChart.chart.spacingRight = 5;
    defaultChart.tooltip.formatter = function () {
        var dateTime = Highcharts.dateFormat('%Y-%m-%dT%H:%M:%S', this.x);
        var unixDateTime = new moment(dateTime + '.0000000Z').valueOf();
    ...
    if(clientStyle.lookup_ChartYAxisInsideOutside === 'inside'){
        defaultChart.yAxis[0].labels = {
            align: 'right',
            x: -5,
            y: -5,
            formatter: function () {
        ...
        defaultChart.yAxis[0].useHTML = true;
        defaultChart.yAxis[0].tickPosition = 'outside';
        defaultChart.yAxis[0].labels = {
            align: 'left',
            x: 2,
            y: -5,
    ...
    // defaultChart.yAxis[0].title = {text: null};
    defaultChart.plotOptions.series = {
        turboThreshold: 1000000,
        animation: {
            duration: 1
        },
    ...
    $(globalChartContainer).highcharts(defaultChart);
    globalChartDom = getChartDOM();
}
