...
Handlebars.registerHelper('lookupCurrency', function () {
    var str = '';
    if(clientStyle.allCurrencies.length > 0) {
        str += '<select class="input-wrapper lookupCurrencyConversion">';
        for (var i = 0; i < clientStyle.allCurrencies.length; i++) {
            var cc = getActiveCurrency() === clientStyle.allCurrencies[i] ? 'selected' : '';
            str += '<option value="' + clientStyle.allCurrencies[i] + '" ' + cc + '>' + clientStyle.allCurrencies[i] + '</option>';
        }
        str += '</select>';
    }
    return str;
