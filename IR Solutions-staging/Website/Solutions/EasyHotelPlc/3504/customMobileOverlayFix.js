...
function loadClickHandlers() {
    ...
    $('.semiTransparantOverlay').on('click', function() {
    ...
    $('.x-close').on('click', function(e){
        e.preventDefault();
        $('.pageVerification').css('display', 'none');
        $('.semiTransparantOverlay').addClass('hidden');
    });
...
function retrieveAndPostValues() {
    ...
    var emailAlertsManagerURL = 'http://ir.euroinvestor.com/Tools/EmailAlertsWithPush/EmailAlertsManager.aspx?ts=' + (new Date).getTime();  
    if (localhost) {
...
function checkInputFields() {
    ...
    if (!isOK) {
        ...
        var reqfields = '#enterInformationEmail, #enterInformationFirstName, #enterInformationLastName';
        
        $('.requiredFieldMSG').html(translationData.data.t_please_verify_all_required_fields + '!');
        $(reqfields).css('border', '2px solid red');
...
function checkReturnValues(data) {
    ...
    if (data.msgToReturn != "") {
        ...
        if (data.unsubscribeEmail == data.enterInformationEmail && isActiveEmailAdress) {
            header = translationData.data.t_success;
            paragraph = translationData.data.t_email_now_unsubscribed;
            html = '<a class="cleanLink goToHome">'+translationData.data.t_back+'</a>';
            $('.formContinue.unsubscribeInTool').hide(); 
        ...
        console.log(data.unsubscribeEmail)
...
$(document).on('keyup', "#enterInformationEmail, .unsubscribeEmail", function (event) {
    
    //Check with email regex if input is valid then update the class
    if (event.target.type === 'email') {
        
        if (emailIsValid(event.target.value) || $(this).val() === '' ) {
            //console.log('valid');
            event.target.classList.remove('failedValidation');
            $(this).parent().find('.messageEmail').css('display', 'none');
        }
        else {
            //console.log('notValid');
            event.target.classList.add('failedValidation');
            $(this).parent().find('.messageEmail').css('display', 'block');
        }
    }
});
