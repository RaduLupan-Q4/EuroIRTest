...
var clientStyle = {
    targetedCurrency: '',
    allCurrencies: ['GBP', 'USD', 'EUR', 'DKK', 'AUD', 'BGN', 'IDR', 'JPN', 'KRW', 'LVL', 'MYR', 'NOK', 'NZD', 'PHP', 'PLN', 'RON', 'RUB', 'SEK', 'SGD', 'THB', 'TRY', 'ZAR'],
    chart_ColourMain: '#03A9F4',
    chart_ColourPlotBackground: '#FFFFFF',
    chart_ColourBackground: '#FFFFFF',
    ...
    chart_plusCurrency: ['GBP', 'USD', 'EUR', 'DKK', 'AUD', 'BGN', 'IDR', 'JPN', 'KRW', 'LVL', 'MYR', 'NOK', 'NZD', 'PHP', 'PLN', 'RON', 'RUB', 'SEK', 'SGD', 'THB', 'TRY', 'ZAR'],
    chart_settingsHighLow: false,
    chart_settingsDrawType: true,
    chart_settingsGraphScale: true,

    lookup_ChartYAxisInsideOutside: 'inside',
    calc_ChartYAxisInsideOutside: 'inside',
    miniquote_ChartYAxisInsideOutside: 'inside',

    amountOfDividentYears: 100,
    amountOfDecimals: 2,
    amountOfTrades: 100,
    ...
    useRealtimeData: false, //not working for now
    useDataSourceRKD: false //not working for now
};
...
var ModulesReady = {
    ...
    activeCount: 0,
    registeredCallbacks: [],
    registeredCallbacksCustom: {},
    triggerCustom: function (funk) {
        this.registeredCallbacks.push(funk);
    },
    triggerCustomModule: function (mod, funk) {
        if (typeof this.registeredCallbacksCustom[mod] === "undefined") this.registeredCallbacksCustom[mod] = [];
        this.registeredCallbacksCustom[mod].push(funk);
    },
    callbackStart: function (mod) {
        if ($.inArray(mod, this.activeModules) != -1 && typeof this.registeredCallbacksCustom[mod] !== "undefined") {
            for (var i = 0; i < this.registeredCallbacksCustom[mod].length; i++) {
                try {
                    this.registeredCallbacksCustom[mod][i]();
                } catch (err) {
                    console.log(err)
                }
            }
        ...
        if (this.activeCount == 0) {
            for (var j = 0; j < this.registeredCallbacks.length; j++) {
                try {
                    this.registeredCallbacks[j]();
                } catch (err) {
                    console.log(err)
                }
            }
...
function initClientStyle() {
    ...
    if (typeof (clientStyleOverwrite) != "undefined") {
        ...
        } else if (typeof ($(IrChartColour).css('stroke')) !== "undefined" && $(IrChartColour).css('stroke') !== null) {
        ...
        } else if (typeof ($(IrChartColour).css('fill')) !== "undefined" && $(IrChartColour).css('fill') !== null) {
        ...
        } else if (typeof ($(IrChartColour).css('color')) !== "undefined" && $(IrChartColour).css('color') !== null) {
        ...
        } else if (typeof ($(IrChartColour).css('background-color')) !== "undefined" && $(IrChartColour).css('background-color') !== null) {
        ...
        } else if (typeof ($(IrChartColour).css('borderTopColor')) !== "undefined" && $(IrChartColour).css('borderTopColor') !== null) {
            clientStyle.chart_ColourBorder = getColor('borderTopColor');
        }
        ...
        if (typeof (clientStyleOverwrite.chart_settingsDrawType) !== "undefined") {
            clientStyle.chart_settingsDrawType = clientStyleOverwrite.chart_settingsDrawType;
        }

        if (typeof (clientStyleOverwrite.chart_settingsGraphScale) !== "undefined") {
            clientStyle.chart_settingsGraphScale = clientStyleOverwrite.chart_settingsGraphScale;
        }
...
function updateTooltipComparisonDC(date) {
    ...
    var tooltipStr = "<div class=\"tooltipHtmlComparison " + globalChartActiveDisplayMode + "Mode\">";
    ...
    if (globalChartActiveDisplayMode == chartDisplayModes.comparison_intraday) {
        ...
        if (dateIndex != -1) {
            tooltipStr += "<div class='IRChartTooltipDate'>" + new moment(globalChartListingStockDataOHLCV[globalActiveListingIndex][dateIndex][0]).format(clientStyle.formatDate) + "</div>";
            ...
            for (var j = 0; j < globalChartComparisonData[0].length; j++) {
                if (globalChartComparisonInChart[j + 1] == 1) {
                    dateIndex = dateIndex > globalChartComparisonData[0][j].length - 1 ? globalChartComparisonData[0][j].length - 1 : dateIndex;
                    tooltipStr += "<div>" +
    ...
    tooltipStr += "</div>";
    var bottom, IRNewsHeadline, newsIndex;
    if (FeaturesList.IRChartNews.use) {
        newsIndex = globalChartNewsDates.indexOf(date);
        if (newsIndex > -1) {
            bottom = -1;
...
function updateChartNavBarRange(module) { //Todo try to check if the switch can be merged
    ...
    if (module == 'IRChartHTMLModule' && typeof ($('.chartChangePeriod')) != "undefined") {
        $('.chartChangePeriod div, .IRChartChangePeriod div').on('click', function () {
            ...
            if (e == 'd1' || e == 'd5') hours = periodSelector(e);
            else days = periodSelector(e);
            updateActiveChartNavBarRangePeriod(e);
            if (days > 0) stateNewHistoricalPeriodSelected(days);
            if (hours > 0) stateNewIntradayPeriodSelected(hours);
        });
        ...
        $('.chartChangePeriod div, .IRChartChangePeriod div').on('click', function () {
            ...
            updateActiveChartNavBarRangePeriod(e);
            if (days > 0) {
                if (globalChartActiveDisplayMode == chartDisplayModes.comparison_intraday) IRChartComparisonFeature.changeBackToHistorical(days);
                else if (globalChartActiveDisplayMode == chartDisplayModes.ta_intraday) IRChartTAfeature.changeBackToHistorical(days);
                else if (globalChartActiveDisplayMode == chartDisplayModes.tsr_intraday) IRChartTSRfeature.changeBackToHistorical(days);
                else  stateNewHistoricalPeriodSelected(days);
            }
            if (hours > 0) {
                if (globalChartActiveDisplayMode == chartDisplayModes.comparison) IRChartComparisonFeature.changeToIntraday(hours);
                else if (globalChartActiveDisplayMode == chartDisplayModes.ta) IRChartTAfeature.changeToIntraday(hours);
                else if (globalChartActiveDisplayMode == chartDisplayModes.tsr) IRChartTSRfeature.changeToIntraday(hours);
                else stateNewIntradayPeriodSelected(hours);
            }
...
function setChartExtremes(mode, period) {
    ...
    if (clientStyle.chart_settingsHighLow && $('.basicButtonLook.selectableItem.highLow').hasClass('active')) IRChartSettingsFeature.addHighLow();
}
...
function getIndexThatBestMatchesDate(array, key, pickedDate, roundUp) {
    ...
    var currentIndex = 0;
    var currentElement;
    var pickedMaxDate = new moment(pickedDate).valueOf();
    while (minIndex <= maxIndex) {
        currentElement = array[currentIndex];
        var currItemDate = new moment(currentElement[key]).valueOf();
        if (currItemDate < pickedMaxDate && currentIndex + 1 <= maxIndex) minIndex = currentIndex + 1;
        else if (currItemDate > pickedMaxDate) maxIndex = currentIndex - 1;
        else return currentIndex;

        currentIndex = Math.ceil((minIndex + maxIndex) / 2);
    }

    if (roundUp && new Date(currentElement[key]) < pickedDate) currentIndex = Math.min(currentIndex + 1, array.length - 1);
    return currentIndex;
}
...
function stockDateToListDate(stockData, _dateProperty) {
    ...
    for (var i = 0; i < stockData.length; i++) {
        var entry = _.clone(stockData[i]);
        entry[dateProperty] = new moment(entry[dateProperty]).format(clientStyle.formatDate);
        transformedData.push(entry);
    }
    return transformedData;
}
function stockDataToChartData(data, _dateProperty, _priceProperty) {
    var transformedData = [],
        dateProperty = _dateProperty || "date",
        priceProperty = _priceProperty || "closePrice";
    for (var i = 0; i < data.length; i++) {
        transformedData.push([new Date(data[i][dateProperty]).getTime(), data[i][priceProperty]]);
    }
...
function ajax_download(url, data) {
    ...
    if (clientCustomerKeyRequired.toLowerCase() == 'pushpay' && /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream) {
        ...
        var $iframe,
            iframe_doc,
            iframe_html;
        if (($iframe = $('#download_iframe')).length === 0) $iframe = $("<iframe id='download_iframe' style='display: none' src='about:blank'></iframe>").appendTo("body");
...
function applyIRNewsPagination() {
    if (clientStyle.paginationShowNumber > 0) {
        $('.IRNewsPaginationPageNew, .IRNewsPageNumber').on('click', function () {
            ...
            } else if ($(this).hasClass('IRNewsPaginationPageFirst') && !$(this).hasClass('inactive')) {
                updateIRNewsPagination(1);
            } else if ($(this).hasClass('IRNewsPaginationPagePrev') && !$(this).hasClass('inactive')) {
                updateIRNewsPagination((parseInt(IRNewsPaginationActivePage - 1)));
            } else if ($(this).hasClass('IRNewsPaginationPageLast') && !$(this).hasClass('inactive')) {
                updateIRNewsPagination(globalNewsPagesInTotal);
...
function updateIRNewsPagination(page) {
    ...
    $('.IRNewsPaginationPageLast, .IRNewsPaginationPageLast').removeClass('inactive');
    ...
    if (maxPage <= clientStyle.paginationShowNumber) {
        ...
        $('.IRNewsPaginationPageLast').removeClass('inactive');
    }
    ...
    if (minPage >= globalNewsPagesInTotal - clientStyle.paginationShowNumber) {
        ...
        $('.IRNewsPaginationPageFirst').removeClass('inactive');
    }

    if (maxPage <= clientStyle.paginationShowNumber) {
        if (currentPage == 1) {
            $('.IRNewsPaginationPagePrev').addClass('inactive');
            $('.IRNewsPaginationPageFirst').addClass('inactive');
        }
    }
    ...
    if (minPage >= globalNewsPagesInTotal - clientStyle.paginationShowNumber) {
        if (currentPage == globalNewsPagesInTotal) {
            ...
            $('.IRNewsPaginationPageLast').addClass('inactive');
        }
    }
...
function IRChartNavigationHideAll() {
    if (typeof ($('.IRChartMenuTrigger')[0]) == 'object') {
...
var IRChartTAfeature = {
    ...
    attachEvents: function () {
        ...
        if (typeof ($('.IRChartTA .IRChartNavigationClearTA')[0]) == 'object') {
...
var IRChartTSRfeature = {
    ...
    attachEvents: function () {
        if (typeof ($('.IRChartTSR .IRChartTSRBodyList .basicButtonLook')[0]) == 'object') {
...
var IRChartCurrencyConverter = {
    ...
    chartCurrencyConverterIntraDayData: function () {
        ...
        var divideFactor = 1;
        if (getActiveCurrency().toLowerCase() == "gbx") divideFactor = 100;
        var workData;
    ...
    attachEvents: function () {
        if (typeof ($('.IRChartCC .IRChartCCBodyList .basicButtonLook')[0]) === 'object') {
...
var IRChartComparisonFeature = {
    ...
    attachEvent: function () {
        if (typeof ($('.IRChartComparison .IRChartComparisonBodyList .basicButtonLook')[0]) == 'object') {
...
var IRChartSettingsFeature = {
    ...
    setChartData: function (type) {
        if (type === "candlestick") {
            if (globalChartActiveDisplayMode == chartDisplayModes.intraday) globalChartDom.series[globalActiveListingIndex].update({data: globalChartListingIntradayDataOHLCV[globalActiveListingIndex]});
            else globalChartDom.series[0].update({data: globalChartListingStockDataOHLCV[globalActiveListingIndex]});
        } else {
            if (globalChartActiveDisplayMode == chartDisplayModes.intraday) globalChartDom.series[globalActiveListingIndex].update({data: globalChartListingIntradayData[globalActiveListingIndex]});
            else globalChartDom.series[0].update({data: globalChartListingStockData[globalActiveListingIndex]});
        }
        globalChartDom.redraw();
    },
    addHighLow: function () {
        debugStep("updateHighLowPlotLineToChartCustom()");
        var extremes = globalChartDom.yAxis[0].getExtremes();
        // High
        ...
        globalChartDom.yAxis[0].addPlotLine({
            ...
            zIndex: 1000,
            label: {
                text: 'High ' + NumberFormat.decimal(extremes.dataMax),
                align: 'left',
                y: -5,
                x: 5,
                width: 0,
        ...
        globalChartDom.yAxis[0].addPlotLine({
            ...
            zIndex: 1000,
            label: {
                text: 'Low ' + NumberFormat.decimal(extremes.dataMin),
                align: 'left',
                y: -5,
                x: 5,
                width: 0,
    ...
    attachEvent: function () {
        ...
        $('.IRChartSettingsBody .basicButtonLook:not(.highLow):not(.IRChartNavigationClearSettings)').click(function () {
        ...
        if (clientStyle.chart_settingsHighLow && !chartEnabledClickHandlers.chartNavigationSettings) {
            $('.IRChartSettingsBody .basicButtonLook.highLow:not(.IRChartNavigationClearSettings)').on('click', function () {
                if (globalChartActiveDisplayMode != chartDisplayModes.historical && globalChartActiveDisplayMode != chartDisplayModes.intraday) {
                    resetIRChart();
                }
                if ($(this).hasClass('active')) {
                    IRChartSettingsFeature.removeHighLow();
                    $(this).removeClass('active');
                } else {
                    IRChartSettingsFeature.addHighLow();
                    $(this).addClass('active');
                }
            })
        }
...
// Calc & Lookup date pickers
var IRdatepickers = {
    readDate: function (form) {
        return new Date(parseInt($("." + form + "-year").val()), parseInt($("." + form + "-month").val()), parseInt($("." + form + "-day").val()));
    },
    setDateSelects: function (date, form) {
        if (!date) return;
        $("." + form + "-day").val(date.getDate());
        $("." + form + "-month").val(date.getMonth());
        $("." + form + "-year").val(date.getFullYear());
        $(".js-" + form).val(moment(date).format(clientStyle.formatDate))
    },
    ...
    swapSelectDates: function () {
        var fromDate = IRdatepickers.readDate('from');
        var toDate = IRdatepickers.readDate('to');
        IRdatepickers.setDateSelects(toDate, 'from');
        IRdatepickers.setDateSelects(fromDate, 'to');
        $(".to-month").val();
        $(".to-year").val();
        $(".to-day").val();
    },
    dateSelectChangeHandler: function () {
        ...
        var validFromDay = Math.min(IRdatepickers.getDaysInMonth(fromMonth, fromYear), fromDay);
        if($(this).hasClass('from-day') || $(this).hasClass('from-month') || $(this).hasClass('from-year')){
            fromMonth = parseInt(scope.find(".from-month").val(), 10) + 1;
            fromYear = parseInt(scope.find(".from-year").val(), 10);
            fromDay = parseInt(scope.find(".from-day").val(), 10);

            toMonth = parseInt($(".to-month").val(), 10) + 1;
            toYear = parseInt($(".to-year").val(), 10);
            toDay = parseInt($(".to-day").val(), 10);

        } else if($(this).hasClass('to-day') || $(this).hasClass('to-month') || $(this).hasClass('to-year')) {
            fromMonth = parseInt($(".from-month").val(), 10) + 1;
            fromYear = parseInt($(".from-year").val(), 10);
            fromDay = parseInt($(".from-day").val(), 10);

            toMonth = parseInt(scope.find(".to-month").val(), 10) + 1;
            toYear = parseInt(scope.find(".to-year").val(), 10);
            toDay = parseInt(scope.find(".to-day").val(), 10);
        }
        validFromDay = Math.min(IRdatepickers.getDaysInMonth(fromMonth, fromYear), fromDay);
        validToDay = Math.min(IRdatepickers.getDaysInMonth(toMonth, toYear), toDay);

        var minFromDate = $(".from-datepicker").datepicker("option", "minDate");
        var minToDate = $(".to-datepicker").datepicker("option", "minDate");

        if (scope.find('.from-year').length !== 0) IRdatepickers.setDateSelects(new Date(fromYear, fromMonth - 1, fromDay), 'from');
        if (scope.find('.to-year').length !== 0) IRdatepickers.setDateSelects(new Date(toYear, toMonth - 1, toDay), 'to');

        if (moment(minFromDate).valueOf() > moment(IRdatepickers.readDate('from')).valueOf()) IRdatepickers.setDateSelects(minFromDate, 'from');
        if (moment(minToDate).valueOf() > moment(IRdatepickers.readDate('to')).valueOf()) IRdatepickers.setDateSelects(minToDate, 'to');

        if (fromDay !== validFromDay) $(".from-day").val(validFromDay);
        if (toDay !== validToDay) $(".to-day").val(validToDay);
        if (IRdatepickers.readDate('to') < IRdatepickers.readDate('from')) IRdatepickers.swapSelectDates();

        // IRdatepickers.restrictSelectDates();

        $(".from-datepicker").datepicker("setDate", IRdatepickers.readDate('from'));
        $(".to-datepicker").datepicker("setDate", IRdatepickers.readDate('to'));
    },
    restrictSelectDates: function (earliest) {
        debugStep("restrictSelectDates");
        var dateControls = ['from', 'to'];
        var now = new Date();
        for (var i = 0; i < dateControls.length; i++) {
            var date = IRdatepickers.readDate(dateControls[i]);
            if (date > now) IRdatepickers.setDateSelects(now, dateControls[i]);
            else if (earliest && date < earliest) IRdatepickers.setDateSelects(earliest, dateControls[i]);
        }
    },
    cleanSelectors: function (fromYear) {
        var tempFromYear = $('.from-year').find('option');
        var tempToYear = $('.to-year').find('option');
        for (var i = 0; i < tempFromYear.length; i++) {
            if ($(tempFromYear[i]).val() < fromYear) $(tempFromYear[i]).remove();
            if ($(tempToYear[i]).val() < fromYear) $(tempToYear[i]).remove();
        }
    },
    ...
    datePickerChangeHandler: function (date, ops) {
        debugStep("datePickerChangeHandler");
        console.log(date, ops)
        var fromDate = $(".from-datepicker").datepicker("getDate"),
            toDate = $(".to-datepicker").datepicker("getDate");
        if (toDate < fromDate) IRdatepickers.swapDatepickerDates();
        if (ops.settings.title === 'to') IRdatepickers.setDateSelects($(".from-datepicker").datepicker("getDate"), 'from');
        else IRdatepickers.setDateSelects($(".to-datepicker").datepicker("getDate"), 'to');
        IRdatepickers.setDateSelects(new Date(date), ops.settings.title);
        $("." + ops.settings.title + "-datepicker").datepicker("setDate", new Date(date));
    },
    datepickerHandlers: function (fromDate, toDate, minDate) {
        $(".date-select").on("change", IRdatepickers.dateSelectChangeHandler);
        $(".from-datepicker").datepicker({
            title: "from",
            showOn: "button",
            buttonImage: getImagePath() + "calendar-gray.svg",
            buttonImageOnly: true,
            onSelect: IRdatepickers.datePickerChangeHandler,
            minDate: new Date(minDate),
            maxDate: new Date()
        });
        $(".from-datepicker").datepicker("setDate", new Date(fromDate));
        ...
        $(".to-datepicker").datepicker("setDate", new Date(toDate));
        $(".js-from, .js-to").on('focus', function () {
            $(this).parent().find('.ui-datepicker-trigger').click();
        })
    },
    cropGraph: function () {
        debugStep("cropGraph");
        globalChartDom.xAxis[0].setExtremes(IRdatepickers.readDate('from'), IRdatepickers.readDate('to'));
    }
};

// Lookup Module
var stockData = [];
var IRLookupModuleBuild = {
    initializeLookup: function (data) {
        debugStep('initializeLookup');
        var toDate = new Date();
        var fromDate = new Date();
        var minDate = null;
        stockData = data.data[globalActiveListingIndex].data;
        if (getUriParam("mode") === "list") {
            IRLookupModuleBuild.populateList();
        } else {
            drawIRChartCalcLookup(stockData);
            fromDate.setFullYear(fromDate.getFullYear() - 1);
            minDate = new Date(stockData[0].date);
            if (moment(fromDate).valueOf() <= moment(stockData[0].date).valueOf()) fromDate = minDate;
            if (minDate.getFullYear() > (new Date().getFullYear() - 10)) IRdatepickers.cleanSelectors(minDate.getFullYear());

            IRdatepickers.setDateSelects(fromDate, 'from');
            IRdatepickers.setDateSelects(toDate, 'to');
            IRdatepickers.datepickerHandlers(fromDate, toDate, minDate);
            if (typeof $(ModulesList.IRLookupModule.listView) !== "undefined" && $(ModulesList.IRLookupModule.listView).css('display') !== 'none') IRLookupModuleBuild.populateTable();
            IRdatepickers.cropGraph();
            IRLookupModuleBuild.calculateHistoricalValue();
            // Event bindings
            $(".date-select").on("change", IRdatepickers.dateSelectChangeHandler);
            $(".date-select").on("change", IRLookupModuleBuild.calculateHistoricalValue);
            $(".lookupCurrencyConversion").on('change', IRLookupModuleBuild.calculateHistoricalValue);
            $(".lookup-form, .lookup-table-form, .lookupLayFour").on("submit", function (e) {
                e.preventDefault();
                IRLookupModuleBuild.submitLookup();
            });
        }
    },
    submitLookup: function () {
        debugStep("submit");
        if (IRLookupModuleBuild.readFormat().toLowerCase() === "excel") {
            ResponseData.requestClosePriceListingData.done(function (closePrices) {
                var startDate = IRdatepickers.readDate('from');
                var endDate = IRdatepickers.readDate('to');
                var frequency = IRLookupModuleBuild.readFrequency();
                var downscaledData,
                    stringified;
                var tableHeader = {
                ...
                downscaledData = ModulesList.IRCustomModule.use ? getCroppedDownscaledData(stockData, startDate, endDate, frequency) : getCroppedDownscaledData(closePrices.data[globalActiveListingIndex].data, startDate, endDate, frequency);
                downscaledData = stockDateToListDate(downscaledData, 'date');
            ...
            if (typeof $(ModulesList.IRLookupModule.listView) !== "undefined" && $(ModulesList.IRLookupModule.listView).css('display') !== 'none') {
                IRLookupModuleBuild.populateTable();
            } else {
                var url = window.location.href;
                url += (url.indexOf("?") === -1) ? "?" : "&";
                url += "mode=list&from=" + IRLookupModuleBuild.readDateLookupResults('from') + "&to=" + IRLookupModuleBuild.readDateLookupResults('to') + "&frequency=" + IRLookupModuleBuild.readFrequency();
    ...
    populateTable: function () {
        ...
        var fromDateRaw = IRLookupModuleBuild.readDateLookupResults('from'),
            toDateRaw = IRLookupModuleBuild.readDateLookupResults('to'),
            frequency = IRLookupModuleBuild.readFrequency() || "daily",
        ...
        $(ModulesList.IRLookupModule.listView).html(compiledIRLookupListTemplate({
            closePrices: downscaledData,
    ...
    populateList: function () {
        ...
        var fromDateRaw = getUriParam("from"),
            toDateRaw = getUriParam("to"),
            frequency = getUriParam("frequency") || "daily",
            fromDate = (fromDateRaw) ? new moment(fromDateRaw)._d : new moment(stockData[globalActiveListingIndex].date)._d,
            toDate = (toDateRaw) ? new moment(toDateRaw)._d : new moment(stockData[stockData.length - 1].date)._d;
        var downscaledData = getCroppedDownscaledData(stockData, fromDate, toDate, frequency);

        $(ModulesList.IRLookupModule.view).html(compiledIRLookupListTemplate({
            closePrices: downscaledData,
            headers: translations
        }));
    },
    readDateLookupResults: function (form) {
        debugStep("readFromDateLookupResults");

        var fromDateM = new moment();
        fromDateM.date(Number($("." + form + "-day").val()));
        fromDateM.month(Number($("." + form + "-month").val()));
        fromDateM.year(Number($("." + form + "-year").val()));
    ...
    readLookupCurrency: function () {
        return $(".lookupCurrencyConversion").val();
    },
    calculateHistoricalValue: function () {
        var to = IRLookupModuleBuild.readLookupCurrency();
        var period = new Date().getFullYear() - IRdatepickers.readDate('to').getFullYear() + 1;
        if (to !== getActiveCurrency() && to !== undefined && to !== null) {
            RequestData.featureCurrencyConversion(getActiveCurrency().replace('GBX', 'GBP'), to, period);
            $.when(ResponseData.requestFeatureCurrencyConversionData)
                .done(function (currencyConversionData) {
                    var weights = currencyConversionData.data;
                    if (weights.length > 0) {
                        var divideFactor = 1;
                        var bestMatchIndex = getIndexThatBestMatchesDate(stockData, "date", IRdatepickers.readDate("to"));
                        var bestWeghtindex = getIndexThatBestMatchesDate(weights, "date", stockData[bestMatchIndex].date);
                        if (getActiveCurrency().toLowerCase() == "gbx") divideFactor = 100;
                        var priceAtFromDate = (stockData[bestMatchIndex].closePrice / divideFactor) * weights[bestWeghtindex].conversationFactor;
                        var openPriceAtFromDate = (stockData[bestMatchIndex].openPrice / divideFactor) * weights[bestWeghtindex].conversationFactor;
                        var dateAtFromDate = stockData[bestMatchIndex].date;
                        var highAtFromDate = (stockData[bestMatchIndex].high / divideFactor) * weights[bestWeghtindex].conversationFactor;
                        var lowAtFromDate = (stockData[bestMatchIndex].low / divideFactor) * weights[bestWeghtindex].conversationFactor;
                        var volumeAtFromDate = stockData[bestMatchIndex].volume;
                        var closePriceAtFromDate = (stockData[bestMatchIndex].closePrice / divideFactor) * weights[bestWeghtindex].conversationFactor;

                        $('.outputLookupValue').html(NumberFormat.decimal1000Zeros(priceAtFromDate));
                        $('.dateHistorPrice').html(moment(dateAtFromDate).format(clientStyle.formatDate));
                        $('.openHistorPrice').html(NumberFormat.decimal1000Zeros(openPriceAtFromDate));
                        $('.highHistorPrice').html(NumberFormat.decimal1000Zeros(highAtFromDate));
                        $('.lowHistorPrice').html(NumberFormat.decimal1000Zeros(lowAtFromDate));
                        $('.volumeHistorPrice').html(NumberFormat.decimal1000Zeros(volumeAtFromDate));
                        $('.closeHistorPrice').html(NumberFormat.decimal1000Zeros(closePriceAtFromDate));
                    }
                })
        } else {
            var bestMatchIndex = getIndexThatBestMatchesDate(stockData, "date", IRdatepickers.readDate("to"));
            var priceAtFromDate = stockData[bestMatchIndex].closePrice;
            var openPriceAtFromDate = stockData[bestMatchIndex].openPrice;
            var dateAtFromDate = stockData[bestMatchIndex].date;
            var highAtFromDate = stockData[bestMatchIndex].high;
            var lowAtFromDate = stockData[bestMatchIndex].low;
            var volumeAtFromDate = stockData[bestMatchIndex].volume;
            var closePriceAtFromDate = stockData[bestMatchIndex].closePrice;
...
var IRCalcModuleBuild = {
    units: {
        SHARES: "shares",
        AMOUNT: "amount",
        fromStringValue: function (stringValue) {
            return _.find(IRCalcModuleBuild.units, function (unit) {
                    return (unit === stringValue);
                }) || null;
        }
    },
    initializeCalc: function (data) {
        debugStep("initializeCalc");
        var toDate = new Date();
        var fromDate = new Date();
        var minDate = null;
        ResponseData.requestClosePriceListingData.done(function (closePriceListingData) {
            stockData = _.clone(closePriceListingData.data[globalActiveListingIndex].data, true);
            stockDataBackup = _.clone(stockData, true);
            ...
            IRdatepickers.dateSelectChangeHandler();
            drawIRChartCalcLookup(stockData);
            ...
            $('input[name=invested]:radio').on("change", function () {
                if (IRCalcModuleBuild.readUnit().toLowerCase() === "amount") $(".currency-symbol").show();
                else $(".currency-symbol").hide();
            });
            $(".calc-form").on("submit", function (e) {
                e.preventDefault();
                IRCalcModuleBuild.submitCalcCheck();
            });
        });
    },
    submitCalcCheck: function () {
        ...
        if (typeof(toCurrency) != "undefined" && toCurrency != "Current") {
            ...
            RequestData.featureCurrencyConversion(fromCurrency.replace('GBX', 'GBP'), toCurrency.replace('GBX', 'GBP'), 10);

            $.when(ResponseData.requestFeatureCurrencyConversionData)
                .done(function (currencyConversionData) {
                    globalRawCurrencyData = currencyConversionData.data;
                    stockData = _.clone(stockDataBackup, true);
                    IRCalcModuleBuild.adjustStockData(fromCurrency.replace('GBX', 'GBP'));
                    IRCalcModuleBuild.submitCalc();
                });
        } else {
            globalActiveCalcCurrency = getActiveCurrency();
            IRCalcModuleBuild.submitCalc();
    ...
    submitCalc: function () {
        debugStep("submit");
        var results = IRCalcModuleBuild.calculate();
        if (typeof (results.buyStockPrice) == 'undefined') {
            IRCalcModuleBuild.setResultsAsNoData();
        } else {
            if (results.buyStockPrice > 0) {
                if (IRCalcModuleBuild.readUnit().toLowerCase() === "amount") $(".est-shares-cell").text(NumberFormat.decimal1000Zeros(results.shares));
                else $(".est-shares-cell").text(NumberFormat.decimal1000(results.shares));

                $(".buy-price-cell").html('<span class="currency-global">' + globalActiveCalcCurrency + '</span> ' + NumberFormat.decimal1000Zeros(results.buyStockPrice));
                $(".sell-price-cell").html('<span class="currency-global">' + globalActiveCalcCurrency + '</span> ' + NumberFormat.decimal1000Zeros(results.sellStockPrice));
                $(".value-now-cell").html('<span class="currency-global">' + globalActiveCalcCurrency + '</span> ' + NumberFormat.decimal1000Zeros(results.sellValue));
                $(".yield-cell").text(NumberFormat.decimal(results.percentReturn) + "%");
                $(".yield-change-cell").text(NumberFormat.decimal1000Zeros(results.sellValue - results.buyValue));

                $('.start-date-cell').text(moment(results.startDate).format(clientStyle.formatDate));
                $('.end-date-cell').text(moment(results.endDate).format(clientStyle.formatDate));

                $(".buy-value-cell").html('<span class="currency-global">' + globalActiveCalcCurrency + '</span> ' + NumberFormat.decimal1000Zeros(results.buyValue));
                $(".value-end-date-cell").text(results.sellStockPrice);
                $(".change-amount-cell").text((results.changeAmmount).toFixed(2));
    ...
    calculate: function () {
        ...
        var amountInvested = parseFloat($(".amount-invested").val(), 10),
            unit = IRCalcModuleBuild.readUnit(),
            fromDate = IRdatepickers.readDate('from'),
            toDate = IRdatepickers.readDate('to'),
            results = {};
        if (amountInvested && stockData) {

            var startIndex = getIndexThatBestMatchesDate(stockData, "date", new Date(fromDate));
            var endIndex = getIndexThatBestMatchesDate(stockData, "date", new Date(toDate));
            var buyStockPrice = parseFloat(stockData[startIndex].closePrice);
            var sellStockPrice = parseFloat(stockData[endIndex].closePrice);
            ...
            results.shares = (unit === IRCalcModuleBuild.units.SHARES) ? amountInvested : amountInvested / buyStockPrice;
            results.buyValue = (unit === IRCalcModuleBuild.units.SHARES) ? results.shares * buyStockPrice : amountInvested;
            results.valueOfHoldingData = IRCalcModuleBuild.calculateValueOfHolding(stockData, null, fromDate, results.shares);
            ...
            results.closePriceArray = IRCalcModuleBuild.outputClosePriceArray(stockData);
            results.startDate = fromDate;
            results.endDate = toDate;
            results.changeAmmount = sellStockPrice - buyStockPrice;
        }

        return results;
    },
    readUnit: function () {
        return IRCalcModuleBuild.units.fromStringValue($("input[name=invested]:checked").val());
    },
    outputClosePriceArray: function (stockData) {
    ...
    calculateValueOfHolding: function (stockData, dividendData, investmentDate, shares) {
        ...
        var dividend = $("select.dividend").val();
        debugStep("dividend: " + dividend);

        switch (dividend) {
            case "cash":
                return IRCalcModuleBuild.calculateValueOfHoldingWithCashDividend(dividendData, stockData, investmentDate, shares);
                break;
            case "reinvest":
                return IRCalcModuleBuild.calculateValueOfHoldingWithReinvestDividend(dividendData, stockData, shares);
                break;
            case "none":
            default:
                return IRCalcModuleBuild.calculateValueOfHoldingWithoutDividend(stockData, shares);
                break;
        }
    },
    calculateValueOfHoldingWithReinvestDividend: function (dividendData, stockData, shares) {
        debugStep("calculateValueOfHoldingWithReinvestDividend");
        return _.map(stockData, function (stockDataEntry) {
            _.each(dividendData, function (dividendDataEntry) {
                if (new moment(dividendDataEntry.date).valueOf() === new moment(stockDataEntry.date).valueOf()) {
    ...
    calculateValueOfHoldingWithCashDividend: function (dividendData, stockData, investmentDate, shares) {
        debugStep("calculateValueOfHoldingWithCashDividend");
        return _.map(stockData, function (stockDataEntry) {
            var dividendReturns = _.map(dividendData, function (dividendDataEntry) {
                if (new moment(dividendDataEntry.date).valueOf < new moment(investmentDate).valueOf() &&
                    new moment(dividendDataEntry.date).valueOf() >= new moment(stockDataEntry.date)) {
                    return dividendDataEntry.dividend * shares;
                }
                return 0;
            });

            var totalDividend = _.reduce(dividendReturns, function (memo, num) {
                return memo + num;
            }, 0);
    ...
    setResultsAsNoData: function () {
        $(".buy-value-cell").text("-");
        $(".start-date-cell").text("-");
        $(".end-date-cell").text("-");
        $(".value-end-date-cell").text("-");
        $(".change-amount-cell").text("-");
        $(".buy-price-cell").text("-");
        $(".est-shares-cell").text("-");
        $(".sell-price-cell").text("-");
        $(".value-now-cell").text("-");
        $(".yield-cell").text("-");
        $(".yield-change-cell").text("-");
    },
    adjustStockData: function (fromCurrency) {
        var stockDataClone = [];
        var divideFactor = 1;
        if (getActiveCurrency().toLowerCase() == "gbx") divideFactor = 100;
    ...
    var moduleHandlers = {
        ...
        IRNewsModule: function () {
            ...
            $('.IRDataGroup .IRDate, .IRDataGroup .IRTitle').on('click', function () {
                ...
                var articleId = $(this).closest('.IRDataGroup');
                if (articleId.length)
                    window.open(getNewsArticlePath() + '?solutionID=' + getSolutionID() + '&customerKey=' + getClientName() + '&storyID=' + articleId.data('article-id') + '&language=' + globalActiveLanguage, '_blank');
            });

            $('.IRDataGroup .IRDownload').on('click', function () {
                debugStep("window.open... PDF link");
                var pdfLink = $(this).find('.iconPDF');
                if (pdfLink.length)
                    window.open(pdfLink.data('link'), '_blank');
            });
