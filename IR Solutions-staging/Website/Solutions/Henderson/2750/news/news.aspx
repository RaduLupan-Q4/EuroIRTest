...
<script type="text/javascript">
    ...
    function applyIRNewsPagination() {
        ...
        if (IRNewsPaginationApplied) {
            ...
            if (typeof ($('.IRNewsPagination').html()) != 'undefined') {
                ...
                globalNewsPagesInTotal = Math.ceil(globalAmountOfNewsItems / (clientStyle.amountOfNewsPerPage - 1));
    ...
    Handlebars.registerHelper('updateNewsCategoriesCustom', function () {
        var cat = ["ACQ", "ARI"];
        //storyid's to be changed to Acquisitions / disposals
        var newsStoryIdACQArr = [12883171, 12882232, 12760482, 12382951];
        //storyid's to be changed to inside information
        var newsStoryIdPOSArr = [13459427, 13459426];


        var listen = setInterval(function () {
            if ($('.IRDataGroup').length > 0) {

                //update category id attr to ACQ
                for (var i = 0; i < newsStoryIdACQArr.length; i++) {
                    $('#' + newsStoryIdACQArr[i] + ' .IRTitle').attr('id', cat[0]);
                }
                //update category id attr to ARI
                for (var i = 0; i < newsStoryIdPOSArr.length; i++) {
                    $('#' + newsStoryIdPOSArr[i] + ' .IRTitle').attr('id', cat[1]);
                }
