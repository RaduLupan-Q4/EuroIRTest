...
function submitCalc() {
    ...
    if (typeof (results.buyStockPrice) == 'undefined') {
        ...
        if (results.buyStockPrice > 0) {
            $(".buy-price-cell").html('<span class="currency-global">'+ globalActiveCalcCurrency + '</span> ' + NumberFormat.decimal(results.buyStockPrice));
            $(".est-shares-cell").text(NumberFormat.decimal(results.shares));
            $(".sell-price-cell").html('<span class="currency-global">'+ globalActiveCalcCurrency + '</span> ' + NumberFormat.decimal(results.sellStockPrice));
            $(".value-now-cell").html('<span class="currency-global">'+ globalActiveCalcCurrency + '</span> ' + NumberFormat.decimal1000(results.sellValue));
...
function initializeCalc() {
    ...
    $(".calc-form").on("submit", submitCalcCheck);
}
//
...
function readFromDate() {
    return new Date(parseInt($(".from-year").val()), parseInt($(".from-month").val()), parseInt($(".from-day").val()));
}
function readToDate() {
    return new Date(parseInt($(".to-year").val()), parseInt($(".to-month").val()), parseInt($(".to-day").val()));
}
