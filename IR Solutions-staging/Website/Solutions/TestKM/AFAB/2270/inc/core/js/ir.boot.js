...
$(function () {
    ...
    if (ModulesList.IRQuoteModule.active) {
        $.when(ResponseData.requestStockData, ResponseData.requestTranslationsData).done(function () {
            if (!waitForAdditionalDataIRQuoteModule) {
                buildQuoteTable();
                formatColour();
                // reportModuleReady('IRQuote');
    ...
    if (ModulesList.IRProfileModule.active) {
        $.when(ResponseData.requestStockData, ResponseData.requestTranslationsData, ResponseData.requestClosePriceListingData).done(function (stockData, translationsData, closePriceListingData) {
            var data = PreloadData.irProfile(stockData, closePriceListingData);
            buildIRProfile(data);
            formatColour();
        });
    }
    ...
    if (ModulesList.IRChartModule.active) {
        ...
        $.when(ResponseData.requestClosePriceOtherData).done(function (closePriceOtherData) {
            FeaturesList.IRChartCompare.use = closePriceOtherData.data.length != 0;
        });

        $.when(ResponseData.requestStockData, ResponseData.requestClosePriceListingData, ResponseData.requestTranslationsData).done(function (stockData, closePriceListingData) {
            ...
            if (FeaturesList.IRChartPressReleaseIRChartHeadline.use) {
                $.when(ResponseData.requestStockData, ResponseData.requestPressReleaseIRChartHeadlineData).done(function (stockData, newsDataInitial) {
                    var o = {
                        headers: translations,
                        data: newsDataInitial[0].data
                    };
            ...
            if (FeaturesList.IRChartPressRelease.use) {
                $.when(ResponseData.requestStockData, ResponseData.requestPressReleaseData).done(function (stockData, newsDataInitial) {
                    var o = {
                        headers: translations,
                        data: newsDataInitial[0].data
                    };
        ...
        $.when(ResponseData.requestStockData, ResponseData.requestIntradayListingData, ResponseData.requestTranslationsData).done(function (stockData, intradayListingData) {
            var o = {
                headers: translations,
                data: {
                    stock: stockData,
    ...
    // if (ModulesList.IRChartHTMLMiniModule.active) {
    //     $.when(ResponseData.requestStockData, ResponseData.requestClosePriceListingData, ResponseData.requestTranslationsData).done(function (stockData, closePriceListingData) {
    //         globalChartContainer = IrChartHtmlMiniPlace;
    //         var o = {
    //             headers: translations,
    //             data: {
    //                 stock: stockData,
    ...
    //             }
    //         };
    //         PreloadData.irChartMiniDataClosePriceListing(o);
    ...
    if (ModulesList.IRMiniquoteChartModule.active) {
        ...
        $.when(ResponseData.requestStockData, ResponseData.requestIntradayListingData, ResponseData.requestTranslationsData).done(function (stockData, intradayListingData) {
            ...
            globalRawStockData = stockData[0].data;
            PreloadData.irMiniquoteChartDataIntradayListing(stockData[0].data, intradayListingData[0].data);
    ...
    //  As an example: ResponseData.requestOrdersData requires the IRInstrumentID from stock data prior to executing the ajax ResponseData.request.
    //
    if (ModulesList.IROrdersModule.active || ModulesList.IRTradesModule.active || ModulesList.IRNewsModule.active || ModulesList.IREmailAlertModule.active || ModulesList.IRNewsHeadlineModule.active || ModulesList.IRCalcModule.active) {
        $.when(ResponseData.requestStockData).done(function () {
            //
            //  Modules
            //
            ...
            if (ModulesList.IRCalcModule.active) {
                if (FeaturesList.IRCalcTSR.use) {
                    ...
                    $.when(ResponseData.requestStockData, ResponseData.requestClosePriceListingData, ResponseData.requestTranslationsData, ResponseData.requestDividendBundle).done(function (stockData, closePriceListingData, translationsData, dividendBundleData) {
