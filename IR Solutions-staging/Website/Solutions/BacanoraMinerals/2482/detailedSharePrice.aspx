...
<% 
    ...
    site.appendCustomCSSFont += @"<link rel=""stylesheet"" href=""//fonts.googleapis.com/css?family=Oxygen:400,700"" type=""text/css"" />";
...
<script id="IRQuoteTableHorizontalTemplate" type="text/x-handlebars-template">
    <div class="IRMiniQuoteQuoteModule">
        <div class="miniquoteDetailsWrapper">
            <div class="Data updated">Last {{headers/t_updated}}: {{showDateWithFormat stocks/timestamp 'DD MMM YYYY HH:MM'}} {{showLocalTimeZoneShort}}</div>
            <div>
                <div class="Data last"><span class="{{showArrow stocks/change}} arrow"></span> {{decimals stocks/last}}</div>
                <div class="currency">{{stocks/currency}}</div>
            </div>
            <div class="Data change">{{headers/t_change}}: <span class="decimals">{{decimals stocks/change}} {{decimals stocks/changePercent}}%</span></div>
        </div>
    </div>
    <h3 class="horizontal-header Header">{{headers/t_exchange_information}}</h3>
    
    <table class="IRDetailedSharePrice table-look horizontal responsive first">
        {{#headers}}
        <tr>
            ...
            <%--<th class="Header currency">{{t_market_cap}}</th>--%>
        </tr>
        {{/headers}}
        {{#stocks}}
        <tr>
            ...
            <td class="Data date">{{showDateWithFormat timestamp 'DD MMM, YYYY HH:MM'}}</td>
            <td class="Data currency">{{currency}}</td>
            <%--<td class="Data currency">£{{showLondonMarketCapM marketCap}}m</td>--%>
        </tr>
        {{/stocks}}
    </table>
    ...
    <table class="IRDetailedSharePrice table-look horizontal responsive">
        ...
        <tr>
            ...
            <th class="Header last">{{t_last}} {{t_close}}</th>
        </tr>
        {{/headers}}
        {{#stocks}}
        <tr>
    ...
    {{#headers}}
    <tr>
        <th class="Header last">{{t_last}} Trade {{t_price}}</th>
    ...
    {{/headers}} {{#stocks}}
    <tr>
        <td class="Data last">{{decimals lastTradePrice}}</td>
   ...
   <div class="iframe-wrapper" style="height: 1208px;">
       <iframe src="http://ir1.euroinvestor.com/asp/ir/BacanoraMinerals/detailedSharePrice.aspx" style="width: 100%; border: none; height: 1208px;"></iframe>
   </div>
</script>
