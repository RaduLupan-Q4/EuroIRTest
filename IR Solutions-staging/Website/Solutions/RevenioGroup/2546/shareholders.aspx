...
<link rel="stylesheet" type="text/css" href="includes_shareholders/css/shareholders.style.css?v=a<%=System.IO.File.GetLastWriteTime(System.Web.HttpContext.Current.Server.MapPath("includes_shareholders/css/shareholders.style.css")).Ticks.ToString()%>" />
<style>
	#selectArchivePeriod {
		margin-left:20px;
	}
...
<script id="IRDataTemplate" type="text/x-handlebars-template">
    <div class="navbar-header">
        <div class="navbar-left">
            <span>{{headers/t_shareholders}}</span>{{{selectPeriod}}}
        </div>
    ...
    <div class="table-wrapper">
        <table class="sortable table-look shareholders tablesaw" data-tablesaw-sortable>
            <tr>
                ...
                <th class="Header" id="name">{{headers/t_shareholder}}</th>
                <th class="Header numberalignment" id="currentHolding" data-sortable-numeric>{{headers/t_shares}}</th>
                <th class="Header numberalignment" data-sortable-numeric>{{headers/t_shares}} %</th>

                <th class="Header numberalignment" data-sortable-numeric>{{headers/t_change}}</th>
                ...
                <th class="Header numberalignment column-last" data-sortable-numeric>% {{headers/t_votes}}</th>
            </tr>
            {{#data}}
                    {{#each shareholderInfoRows}}
                    <tr>
                        <td class="Data column-first">{{rowID @index}}</td>
                        <td class="Data">{{shareholder}}</td>
                        {{#each holdings}}
                        <td class="Data numberalignment">{{toLocal aShares}}</td>
                        <td class="Data numberalignment">{{decimals sharesPct}}</td>

                        <%--  <td class="Data">{{aChange}}</td>
                        <td class="Data">{{bShares}}</td>
                        <td class="Data">{{bChange}}</td>--%>
                        <td class="Data numberalignment">{{aChange}}</td>
                        <td class="Data numberalignment column-last">{{decimals votesPct}}</td>
                        {{/each}}
                        <%--{{{holdings id}}}--%>
                    </tr>
            {{/each}}
    ...
    <form method="POST" id="excelform" action="/ServiceEngine/api/json/reply/RequestExcel" target="_blank">
        ...
        <div style="margin-top: 20px; font-weight: bold; line-height: 35px;">
            <input type="button" onclick="submittable()" value=""
                style="background: none; border: none; background-image: url('images/MSexcelIcon.ico'); float: left; padding: none; background-repeat: no-repeat; width: 35px; background-size: 100%; background-repeat: no-repeat; height: 35px; border-radius:0px;">&nbsp;&nbsp;{{headers/t_download_excel}}
        </div>
