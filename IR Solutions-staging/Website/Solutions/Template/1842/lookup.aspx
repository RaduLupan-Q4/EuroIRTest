...
<script id="IRLookupTableTemplate" type="text/x-handlebars-template">
    <table class="IRLookupResultsTable table-look horizontal responsive">
        <tr>
            ...
            <th class="Header openPrice">{{headers/t_open}}</th>
            <th class="Header high">{{headers/t_high}}</th>
            <th class="Header low">{{headers/t_low}}</th>
            <th class="Header closePrice">{{headers/t_close}}</th>
            <th class="Header volume column-last">{{headers/t_volume}}</th>
        </tr>
        {{#each closePrices}}
            <tr>
                <td class="Data date column-first">{{date}}</td>
                <td class="Data openPrice">{{decimal openPrice 2}}</td>
                <td class="Data high">{{decimal high 2}}</td>
                <td class="Data low">{{decimal low 2}}</td>
                <td class="Data closePrice">{{decimal closePrice 2}}</td>
                <td class="Data volume column-last">{{volume}}</td>
            </tr>
        {{/each}}
    </table>
</script>
