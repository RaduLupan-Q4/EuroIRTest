function isDev() {
    if (location.href.indexOf('localhost') > -1 || location.href.indexOf('http://devir.euroinvestor.com/') > -1 || location.href.indexOf('file:') > -1) {
...
var IRChartTSRfeature = {
    ...
    applyTSR: function (TSRMode) {
        ...
        IRChartTSRfeature.stockDataClone = globalChartListingStockData[globalActiveListingIndex];
        switch (TSRMode) {
            case "TSRSimple":
    ...
    applySimpleTSRToChart: function () {
        ...
        IRChartTSRfeature.dividendSimpleData = [];
        var colourIndex = 1;
        ...
        for (var i = 0; i < tsrDividend.length; i++) {
            ...
            for (j; j < tempStockData.length; j++) {
                ...
                tempTSRsimple.push(tempStockData[j][1] + k);
                IRChartTSRfeature.dividendSimpleData.push([tempStockData[j][0], tempTSRsimple[j] / tempTSRsimple[0] * 100]);
        ...
        IRChartTSRfeature.applyTSRnewData(IRChartTSRfeature.dividendSimpleData, globalChartColours[colourIndex], 'dividendSimpleData');
        // initTSRUpdateInterval();
    },
    applyReinvestTSRToChart: function () {
        debugStep("applyReinvestTSRToChart");
        IRChartTSRfeature.dividendReinvestData = [];
        var colourIndex = 2;
        var tsrDividend = IRChartTSRfeature.dividentBundleData;
        var tempTSRreinvest = [];
        var tempStockData = globalChartListingStockData[globalActiveListingIndex];
        ...
        for (var i = 0; i < tsrDividend.length; i++) {
            ...
            for (j; j < tempStockData.length; j++) {
                ...
                tempTSRreinvest.push(tempStockData[j][1] * sO);
                IRChartTSRfeature.dividendReinvestData.push([tempStockData[j][0], tempTSRreinvest[j] / tempTSRreinvest[0] * 100]);
                if (IRChartTSRfeature.dividendDatesInChart.length != tempStockData.length) {
                    IRChartTSRfeature.dividendDatesInChart.push(tempStockData[j][0]);
                }
            }
        }
        IRChartTSRfeature.applyTSRnewData(IRChartTSRfeature.dividendReinvestData, globalChartColours[colourIndex], "dividendReinvestData");
        // initTSRUpdateInterval();
    },
    applyTSRnewData: function (data, color, name) {
        globalChartDom.addSeries({
            index: globalChartDom.series.length,
            zIndex: 10,
            data: data,
            color: color,
            yAxis: 0,
...
// Todo KM bring back commented functions in final version
...
function getTooltipStrSubTSR(dateIndex) {
    var tooltipStr = "";
    tooltipStr += "<div class=\"IRChartTooltipDate\">" + new moment(IRChartTSRfeature.dividendSimpleData[dateIndex][0]).format(clientStyle.formatDate) + "</div>";
    tooltipStr += "<div class=\"IRChartTooltipClose\"><span class=\"subHeader\">" + getOHLCfromTranslations('c') + "</span><span class=\"subContent\">" + formatDecimal(globalChartListingStockData[globalActiveListingIndex][dateIndex][1]) + "</div>";
    for (var i = 0; i < IRChartTSRfeature.activeTSR.length; i++) {
        tooltipStr += "<div><span class=\"subHeader\"><span class=\"colorIndicator color" + globalChartColours[i + 1].replace('#', '') + " active\"></span>" + IRChartTSRfeature.activeTSR[i].replace("Data", "").replace("dividend", "") + ": </span><span class=\"subContent extraSpace\">" + formatDecimal(IRChartTSRfeature[IRChartTSRfeature.activeTSR[i]][dateIndex][1]) + "</span></div>";
    }
    tooltipStr += "<div class=\"IRChartTooltipVolume\"><span class=\"subHeader\">" + getOHLCfromTranslations('v') + "</span><span class=\"subContent\">" + formatLocal(globalChartListingStockDataOHLCV[globalActiveListingIndex][dateIndex][5]) + "</span></div>";
    return tooltipStr;
}
function getTooltipStrSubTSR_backup(dateIndex) {
    //var tooltipStr = "";
    //if (globalChartUseCustomTooltipContent && clientStyle.chart_TooltipHideDate) { }
    //else {
    //    try {
    ...
    //if (globalChartUseCustomTooltipContent && clientStyle.chart_TooltipHideClose) { }
    //else {
    //    if (typeof (IRChartTSR.stockDataCloneClose) == 'object') {
    //        if (typeof (IRChartTSR.stockDataCloneClose[dateIndex]) == 'object') {
    ...
    //        } else {
    //            tooltipStr += "<div class=\"IRChartTooltipClose\"><span class=\"subHeader\">" + getOHLCfromTranslations('c') + "</span><span class=\"subContent\">-</div>";
    //        }
    //    }
    //}
    //if (globalChartUseCustomTooltipContent && clientStyle.chart_TooltipHideClose) { }
    //else {
    //    if (typeof (IRChartTSR.stockDataTSRClone) == 'object') {
    //        if (typeof (IRChartTSR.stockDataTSRClone[dateIndex]) == 'object') {
    //            try {
    //                tooltipStr += "<div class=\"IRChartTooltipTSR\"><span class=\"subHeader\">" + translations.t_tsr + "</span><span class=\"subContent\">" + formatDecimal(IRChartTSR.stockDataTSRClone[dateIndex][1]) + "</div>";
    //            }
    //            catch (err) {
    //                tooltipStr += "<div class=\"IRChartTooltipTSR\"><span class=\"subHeader\">" + translations.t_tsr + "</span><span class=\"subContent\">-</div>";
    //            }
    //        } else {
    //            tooltipStr += "<div class=\"IRChartTooltipTSR\"><span class=\"subHeader\">" + translations.t_tsr + "</span><span class=\"subContent\">-</div>";
    //        }
            ...
            if (typeof ($('.IRChartTA #IRChartNavigationClearTA')) == 'object') {
                $('.IRChartTABodyList .basicButtonLook').click(function () {
                    if ($(this).not('#IRChartNavigationClearTA')) {
                        ...
                        if ($(this).hasClass('active')) {
                        ...
                        } else if(!$(this).hasClass('pop') && !$(this).hasClass('active') && typeId == 'SimpleMovingAverage'){
                            var temp = '<div class="chart-popup" data-poptext="'+typeId+'" data-popcol="'+$(this).data('color')+'">' +
                                '<span class="pop-title">' + $(this).text() + '</span>' +
                                '<span class="pop-close">x</span>' +
                                    '<label>Period' +
                                    '<select class="period-list">'+
                                        '<option>10</option>'+
                                        '<option>20</option>'+
                                    '</select>'+
                                    '<span class="select-arrow"></span>'+
                                '</label>'+
                                    '<button type="button" class="pop-submit">Add</button>'+
                                '</div>';
                            $('.IRChartPlaceholderArea').append(temp);
                            $('.IRChartPlaceholderArea .chart-popup').slideDown(300);
                            $(this).addClass('pop');
                        }else if(!$(this).hasClass('pop')){
                            $('.IRChartPlaceholderArea .chart-popup').remove();
                            $('.IRChartTA').find('.basicButtonLook').removeClass('pop');
                            if (IRChartCurrencyConverter.isCurrencyActive()) {
                            ...
                            updateTechnicalAnalysis(typeId, colorId);
                            $(this).addClass('active');
                        }
                    }
                });
                $(document).on('click', '.pop-close', function () {
                    var typeId = $('.IRChartPlaceholderArea .chart-popup').data('poptext');
                    $('.IRChartPlaceholderArea .chart-popup').slideUp(300, function () {
                        $('.IRChartPlaceholderArea .chart-popup').remove();
                        $('.IRChartTA').find('*[data-type="'+typeId+'"]').removeClass('pop');
                    });
                });
                $(document).on('click', '.pop-submit', function () {
                    var typeId = $('.IRChartPlaceholderArea .chart-popup').data('poptext');
                    var colorId = $('.IRChartPlaceholderArea .chart-popup').data('popcol');
                    var period = $('.IRChartPlaceholderArea .chart-popup .period-list').val();
                    $('.IRChartPlaceholderArea .chart-popup').slideUp(300, function () {
                        $('.IRChartPlaceholderArea .chart-popup').remove();
                        updateTechnicalAnalysis(typeId, colorId, period);
                        $('.IRChartTA').find('*[data-type="'+typeId+'"]').addClass('active');
                        $('.IRChartTA').find('*[data-type="'+typeId+'"]').removeClass('pop');
                    });
                });
...
function getClosestDateIndexForListingTSR(unixDate) {
    ...
    for (var i = 0; i < IRChartTSRfeature.dividendDatesInChart.length - 1; i++) {
        if (newUnixDate < unixDate) {
            newUnixDate = IRChartTSRfeature.dividendDatesInChart[i];
            iterations++;
        }
    }
    //}
...
function newsFilter(searchText) {
    ...
    if (typeof ($('div.IRNewsTool .IRHeaderGroup .IRHeader').html()) != "undefined") {
        ...
        $('div.IRDataGroup').each(function () {
            ...
            var newsDate = Date.parse($(this).find('.IRDate').attr('id'));
    ...
    if (typeof ($('table.IRNewsTool tr th.Header').html()) != "undefined") {
        ...
        $('tr.Data').each(function () {
            ...
            var newsDate = Date.parse($(this).find('.date').attr('id'));
...
function IRChartNavigationCloseOpenBodyDivs() {
    $('.IRChartComparisonBody, .IRChartTABody, .IRChartTSRBody, .IRChartCCBody').css('display', 'none');
}
    ...
    switch (globalChartActiveDisplayMode) {
        case chartDisplayModes.historical:
            try {
                ...
                tooltipStr += "<div><span class=\"subHeader\">" + getOHLCfromTranslations('c') + "</span><span class=\"subContent\">" + formatDecimal(globalChartListingStockDataOHLCV[globalActiveListingIndex][dateIndex][4]) + "</span></div>";
                tooltipStr += "<div><span class=\"subHeader\">" + getOHLCfromTranslations('v') + "</span><span class=\"subContent\">" + formatLocal(globalChartListingStockDataOHLCV[globalActiveListingIndex][dateIndex][5]) + "</span></div>";
            }
            catch (err) {
                dateIndex = getClosestDateIndexForListingClosePrice(date);
                tooltipStr += "<div>" + new moment(globalChartListingStockDataOHLCV[globalActiveListingIndex][dateIndex][0]).format(clientStyle.formatDate) + "</div>";
            ...
            if (useIRChartTA) {
                ...
                try {
                    for (var i = 0; i < activeTA.length; i++) {
                        ...
                        tooltipStr += "<div><span class=\"subHeader\"><span class=\"colorIndicator color" + activeTAcolor[i].replace('#', '') + " active\"></span>" + activeTA[i] + ": </span><span class=\"subContent\">" + formatDecimal(activeTAlist[i][dateIndex][1]) + "</span></div>";
                    }
                }
                catch (err) {
                    dateIndex = getClosestDateIndexForListingClosePrice(date);
                    for (var j = 0; j < activeTA.length; j++) {
                        tooltipStr += "<div><span class=\"subHeader\"><span class=\"colorIndicator color" + activeTAcolor[j].replace('#', '') + " active\"></span>" + activeTA[j] + ": </span><span class=\"subContent\">" + formatDecimal(activeTAlist[j][dateIndex][1]) + "</span></div>";
                    }
                }
            }
        ...
        case chartDisplayModes.intraday:
            try {
                ...
                tooltipStr += "<div><span class=\"subHeader\">" + getOHLCfromTranslations('h') + "</span><span class=\"subContent\">" + formatDecimal(globalChartListingIntradayDataOHLCV[globalActiveListingIndex][dateIndex][2]) + "</span></div>";
                tooltipStr += "<div><span class=\"subHeader\">" + getOHLCfromTranslations('l') + "</span><span class=\"subContent\">" + formatDecimal(globalChartListingIntradayDataOHLCV[globalActiveListingIndex][dateIndex][3]) + "</span></div>";
                tooltipStr += "<div><span class=\"subHeader\">" + getOHLCfromTranslations('c') + "</span><span class=\"subContent\">" + formatDecimal(globalChartListingIntradayDataOHLCV[globalActiveListingIndex][dateIndex][4]) + "</span></div>";

                tooltipStr += "<div><span class=\"subHeader\">" + getOHLCfromTranslations('v') + "</span><span class=\"subContent\">" + formatLocal(globalChartListingIntradayDataOHLCV[globalActiveListingIndex][dateIndex][5]) + "</span></div>";
            }
            catch (err) {
        ...
        switch (TAName) {
            ...
            case "CommodityChannelIndex":
                $.when(requestAnalysisCommodityChannelIndexData).done(function (dataTA) {
                    applyAnalysisToChart(dataTA, TAShort, color, 'dual');
                });
                break;
            case "DirectionalMovementIndex":
...
function applyAnalysisToChart(dataTA, TAShort, color, TAtype, period) {
    ...
    globalChartDom.redraw();
    var numb = '';
    if (typeof period != 'undefined'){ // SMA period number
        numb = '('+period+')';
    }
...
function resetIRChartNavigation() {
    ...
    if (globalChartActiveDisplayMode != chartDisplayModes.comparison) {
        ...
        $('.IRChartComparisonPlaceholder').html('');
...
var IRChartTechnicalAnalysisControls = {
    ...
    updateTechnicalAnalysis: function (id) {
        debugStep("IRChartTechnicalAnalysisControls.updateTechnicalAnalysis");
        ...
        switch (id) {
            case "IRChartTASMA":
                ...
                IRChartTechnicalAnalysis.checkSMA('SMA', 10);
                break;
        }
...
var IRChartTechnicalAnalysis = {
    ...
    drawIRChartTechnicalAnalysis: function (chartMode) {
        ...
        switch (chartMode) {
            // case "single":
            //     $(globalChartContainer).highcharts('StockChart').addSeries({name: title, data: temp});
            //
            ...
            //     break;
            case "dual":
                // $(globalChartContainer + '2.secondary').html();
                // $(globalChartContainer + '2.secondary').highcharts('StockChart', {
                //     colors: [clientStyle.chart_ColourMain, '#002395', '#03C03C', '#0F52BA', '#138808', '#3B444B', '#4B5320', '#507D2A', '#556B2F', '#FF0000', '#779ECB', '#915C83', '#91877D', '#FF00DC', '#967117', '#9932CC', '#00FF90', '#A4C639', '#B2EC5D', '#CBA135', '#CD9575', '#91877D', '#967117', '#FF8C00', '#779ECB', '#A4C639'],
                //     chart: {
                //         alignTicks: true,
                //         borderWidth: 1,
    ...
    updateTooltip: function (date) {
        ...
        switch (globalActiveTAShort) {
            ...
            case "RSI":
                try {
                    dateIndex = stockDataTADates.indexOf(date);
                    tooltipStr += "<div>" + getActiveTAShort() + ": " + formatDecimal(IRChartTechnicalAnalysisData.stockDataTARSI[globalActiveListingIndex][dateIndex][1]) + "</div>";
                }
                catch (err) {
                    dateIndex = getClosestDateIndexForListingClosePrice(date);
                    tooltipStr += "<div>" + getActiveTAShort() + ": " + formatDecimal(IRChartTechnicalAnalysisData.stockDataTARSI[globalActiveListingIndex][dateIndex][1]) + "</div>";
            ...
            case "Momentum":
                try {
                    dateIndex = stockDataTADates.indexOf(date);
                    tooltipStr += "<div>" + getActiveTAShort() + ": " + formatDecimal(IRChartTechnicalAnalysisData.stockDataTAMomentum[globalActiveListingIndex][dateIndex][1]) + "</div>";
                }
                catch (err) {
                    dateIndex = getClosestDateIndexForListingClosePrice(date);
                    tooltipStr += "<div>" + getActiveTAShort() + ": " + formatDecimal(IRChartTechnicalAnalysisData.stockDataTAMomentum[globalActiveListingIndex][dateIndex][1]) + "</div>";
    ...
    applyMomentum: function (TAShort, settingPeriod) {
        $.when(requestAnalysisMomentumData).done(function (data) {
            ...
            // resultUpper
            $(globalChartContainer + '2.secondary').highcharts().addSeries({
                id: 42,
                index: 1,
