...
<script id="IRNewsEntriesTemplate" type="text/x-handlebars-template">
    ...
    <div>
        ...
        <div class="contIrNews">
                <div class="IRNewsTableFooter">
                   <div class="PagePrev">Prev</div>
                    <div class="IRNewsPagination"></div>
                    <div class="PageNext">Next</div>
        ...
        if (IRNewsPaginationApplied) {}
            ...
            if (typeof ($('.IRNewsPagination').html()) != 'undefined') {
                ...
                $(function () {
                    ...
                    //==============New Function for showing data from current year and all years =============

                    

                    //-------Set current year
                    var output = new Date().getFullYear();
                    
                    //------Show current years
                    $('#from-year').val(output);
                    $('#to-year').val(output);
                    console.log($('#from-year').val());
                    console.log($('#to-year').val());
                    $(".newsSubmit").trigger('click');
                    
                    //-------Button "All Years" after click show the data from all years
                    $('#tab2').on('click', function () {
                        
                        //Push to array value of years
                        var arrForYear = [];
                        $('#from-year option').each(function () {
                            arrForYear.push($(this).val());
                        });
                        console.log(arrForYear);
                        var forToYear = arrForYear[arrForYear.length - 1]
                        console.log(forToYear);
                        //Set search from first year to last year
                        $('#from-year').val(arrForYear[0]);
                        $('#to-year').val(forToYear);
                        $(".newsSubmit").trigger('click');
                        
                        var arrPages = [];
                        $('.IRNewsPagination div').each(function(){
                            arrPages.push(this);
                               
                        });
                        console.log(arrPages);
                        $('.IRNewsPageNumber').on('click', function(){
                        if ( $(arrPages[0]).hasClass('active') ) {
                        $('.PagePrev').css('display', 'none');
                        }else{
                        $('.PagePrev').css('display', 'inline-block');
                        };
                        
                        if ($(arrPages[arrPages.length-1]).hasClass('active')){
                            $('.PageNext').css('display', 'none');
                            }else{
                            $('.PageNext').css('display', 'inline-block');
                            }
                    });
                        
                    //Counter for next/prev
                    var prev = $('.PagePrev');
                    var next = $('.PageNext');
                    var counter = 0;
                    //console.log(arrPages[counter]);

                    $(prev).on('click', function(){
                        counter--;

                            console.log(counter)
                            $(arrPages[counter]).trigger('click');
                    ...
                    //-------Show Current Years with data after click on the button of years
                ...
                //Click event for search button after DOM is loaded
                $(".newsSubmit").trigger('click');
                ...
                //============End Of New Function for showing data from current year and all years =============
