...
<script id="IRLookupTemplate" type="text/x-handlebars-template">
    ...
    <div class="lookup-result-wrapper" style="display: none">
        ...
        <div class="chart-wrapper">
            ...
            <div class="IRChartCurrencyLookupPlaceholder"
        </div>
    </div>
...
<script>
                ...
                $.when(requestClosePriceListingData)
                    .done(function (data) {
                        ...
                        $('#showSharePriceButton').click(function () {
                            ...
                            //publish new chart
                            //var filteredClosePriceDataWithCurrency = filterCurrencyData(startDate, endDate, globalClosePriceData);
                            //populateLookUpChart(filteredClosePriceDataWithCurrency);
                            ...
                            if (typeof ($('#currencyConversionDL').val()) != "undefined" && $('#currencyConversionDL').val() != "Current") {
                                ...
                                if ($('#currencyConversionDL').val() == 'USD') {
                                    ...
                                    filteredClosePriceDataWithCurrency = filterCurrencyData(startDate, endDate, globalClosePriceData);
                                    filteredData = filteredClosePriceDataWithCurrency;
                                    populateLookUpChart(filteredClosePriceDataWithCurrency);
                                ...
                                if ($('#currencyConversionDL').val() == 'GBX') {
                                    filteredData = filterCurrencyData(startDate, endDate, globalClosePriceDataRAW);
                                    populateLookUpChart(filteredData);
                                }
                            } else {
                                filteredData = filterCurrencyData(startDate, endDate, globalClosePriceDataRAW);
                                populateLookUpChart(filteredData);
                            ...
                            $('.periodDataValues').html(
                                "<table class='periodValuesTable'><tbody>" +
                                    ...
                                    "<tr>" +
                                        ...
                                        "<td class='period-data'>" + formatDecimalDecimal1000_fixed(highestClosePriceObj.closePrice) + "</td>" +
                                        "<td class='period-date'>" + moment(highestClosePriceObj.date).format('DD/MM/YYYY') + "</td>" +
                                    "</tr>" +
                                    "<tr>" +
                                        "<td class='period-header'>Lowest Price</td>" +
                                        "<td class='period-data'>" + formatDecimalDecimal1000_fixed(lowestClosePriceObj.closePrice) + "</td>" +
                        ...
                        //filter data by selected period and currency
                        function filterCurrencyData(startDate, endDate, array) {
                            var filteredData = array.filter(function (item) {
                        ...
                        function populateLookUpChart(closePriceListingData) {
                            debugStep("populateLookupChart");
                            ...
                            //$("#from-datepicker").datepicker("option", "minDate", new Date(stockData[globalActiveListingIndex].date));
                            //$("#to-datepicker").datepicker("option", "minDate", new Date(stockData[globalActiveListingIndex].date));
                            //$("#from-datepicker").datepicker("option", "minDate", maxData);
                            //$("#to-datepicker").datepicker("option", "maxDate", maxData);
                            transformedData = stockDataToChartData(closePriceListingData);
                            $('.IRChartCurrencyLookupPlaceholder').highcharts({
    ...
    function getDateOrClosestPreviousDate(array, key, ticks) {
        var t = ticks;
        var i = 0;
        while (!array[t] && i < 300) {
            t -= 86400000;//sub 24 hours
            i++;
        }
        if (i === 300) return 0;
        return array[t];
    ...
    function adjustStockData(currencyCrossName, adjustForPoundPence, stockData, currencyData) {
        ...
        $.each(currencyData, function (index, item) {
            currencyDictionary[new Date(item.date).getTime()] = item;
        });
        ...
        $.each(stockData, function (index, item) {
            ...
            var currency = getDateOrClosestPreviousDate(currencyDictionary, "date", new Date(item.date).getTime());

            stockDataElement.date = item.date;
            stockDataElement.closePrice = (item.closePrice / divideFactor) * currency.conversationFactor;
            stockDataElement.openPrice = (item.openPrice / divideFactor) * currency.conversationFactor;
            stockDataElement.high = (item.high / divideFactor) * currency.conversationFactor;
            stockDataElement.low = (item.low / divideFactor) * currency.conversationFactor;
