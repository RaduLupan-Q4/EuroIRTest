...
if (language == undefined || language == 'en') {
    ...
    $.getJSON('http://ir.euroinvestor.com/serviceengine/api/json/reply/RequestAnnouncement?lcid=2057&apiVersion=1&solutionID=1804&CustomerKey=AlmBrand&InstrumentID=100059', function (data) {
        ...
        var latestPublishYear = newDateFormat(getLastPublishYear(allData, "publishTime"));
        ...
        $(allData.data).each(function () {
            ...
            var date = new Date(newDateFormat(this.publishTime)); //get Modified timestamp from JSON
...
function filterData(startDate, endDate, dataArray) {
    var filteredData = dataArray.filter(function (item) {
        ...
        if (new Date(newDateFormat(item.publishTime)) >= startDate && new Date(newDateFormat(item.publishTime)) <= endDate) {
            return item;
        }
    });
    return filteredData;
...
function newDateFormat(date){ //This is for IE, because IE doesnt understand date format
    var nwDate, temp, bigDate, smallDate;
    temp = date.split("T");
    bigDate = temp[0].split('-');
    smallDate = temp[1].split('.')
    nwDate = bigDate[1]+'/'+bigDate[2]+'/'+bigDate[0]+' '+smallDate[0];
    return nwDate;
}