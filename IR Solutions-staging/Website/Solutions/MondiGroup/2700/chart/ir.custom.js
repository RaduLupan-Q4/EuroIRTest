...
function updateTooltipTechnicalAnalysisCustom(date) {
    ...
    try {
        ...
        tooltipStr += "<div><span class=\"subHeader\">" + translations.t_v + "</span><span class=\"subContent\">" + formatLocal(globalChartListingStockDataOHLCV[globalActiveListingIndex][dateIndex][5]) + "</div>";
...
var drawChart = {
    ...
    drawBasicChartHistorical: function () {
        ...
        //insert translations for hardcoded values
        $('.t_comparison').html(translationData.data.t_comparison);
        $('.t_hang_seng_index').html(translationData.data.t_hang_seng_index);


        $('.legendOpen').html(getOHLCfromTranslations('o') + ' - ' + translationData.data.t_open);
        $('.legendHighest').html(getOHLCfromTranslations('h') + ' - ' + translationData.data.t_highest);
        $('.legendLowest').html(getOHLCfromTranslations('l') + ' - ' + translationData.data.t_lowest);
        $('.legendClose').html(getOHLCfromTranslations('c') + ' - ' + translationData.data.t_close);
        $('.legendVolume').html(translationData.data.t_v + ' - ' + translationData.data.t_volume);
        ...
        if (globalActiveLanguage == 'zh-t') {
            //var dateFormat = translateMonthShort(ts.format("%Y")) + translations.t_year +' ' + '%b';
            var dateFormat = translateMonthShort(ts.format("%Y") + translations.t_year);
        } else {
            dateFormat = '%Y';
        }
    ...
    drawBasicChartIntraday: function () {
        ...
        if (globalActiveLanguage == 'zh-t') {
            var dateFormat = translateMonthShort(globalChartListingHistoricalLastKnownMomentDate.format("M")) + translations.t_month + '%d' + translations.t_day;
        } else {
            dateFormat ='%d.'+globalChartListingHistoricalLastKnownMomentDate.format("MMM");
        }
...
function drawIRChartHTMLCompare() {
    ...
    $('.IRChartChangePeriodCustom').find('.activePeriod').removeClass('activePeriod');
    $('#y1').addClass('activePeriod');
}
