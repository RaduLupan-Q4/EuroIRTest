...
<script type="text/javascript">
    ...
    function removeDecimalsAndAddThousands(){
        var separ = ",";
        $('.highcharts-container .highcharts-yaxis-labels').find('text').each(function(){
            var nb = $(this).text().toString().split(".");
            $(this).text(nb[0].toString().replace(/\B(?=(\d{3})+(?!\d))/g, separ));
        });
        var chartBox = $('.highcharts-container .chartLastPrice').text().toString().split(".");
        $('.highcharts-container .chartLastPrice').text(chartBox[0].toString().replace(/\B(?=(\d{3})+(?!\d))/g, separ));
    }
