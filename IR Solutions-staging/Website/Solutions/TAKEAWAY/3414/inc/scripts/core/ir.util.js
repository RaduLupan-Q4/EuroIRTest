...
var globalRawClosePriceBundlePreload = null;
//
...
var clientStyle = new function () {
    ...
    this.chart_VolumeYaxis = false;
    this.chart_VolumeHeight = 40;

    this.chart_settingsHighLow = false;
    this.chart_settingsDrawType = true;
    this.chart_settingsGraphScale = true;
...
$(function () {
    ...
    if (IRLookupModule) {
        ...
        $.when(requestStockData, requestClosePriceListingData, requestTranslationsData).done(function (stockData, closePriceListingData) {
            ...
            globalRawClosePriceBundlePreload = o;
...
function initClientStyle() {
    ...
    if (typeof (clientStyleOverwrite) != "undefined") {
        ...
        if (typeof (clientStyleOverwrite.chart_VolumeYaxis) != "undefined"){
            clientStyle.chart_VolumeYaxis = clientStyleOverwrite.chart_VolumeYaxis;
        }
        if (typeof (clientStyleOverwrite.chart_VolumeHeight) != "undefined"){
            clientStyle.chart_VolumeHeight = clientStyleOverwrite.chart_VolumeHeight;
        }
...
function initMomentTimezone() {
    ...
    switch (globalRawStockData[globalActiveListingIndex].exchangeName) {
        ...
        case 'Mexico Variable Income (Equities)':
            ...
            break;
        case 'Philippine Stock Exchange':
            moment.tz.add('Asia/Manila|PHT PHST JST|-80 -90 -90|010201010|-1kJI0 AL0 cK10 65X0 mXB0 vX0 VK10 1db0|24e6');
            moment.tz.link('Asia/Manila|Philippine');
            globalActiveExchangeTimeZone = 'Asia/Manila';
            break;
        default:
...
function attachClickHandlers(module) {
    ...
    if (useIRChartSettings) {
        ...
        $('.IRChartSettingsBody .basicButtonLook:not(.highLow)').click(function () {
            ...
            if (type == 'candlestick') {
                if (globalChartActiveDisplayMode == chartDisplayModes.intraday) globalChartDom.series[0].update({ data: globalChartListingIntradayDataOHLCV[0] });
                else globalChartDom.series[0].update({ data: globalChartListingStockDataOHLCV[0] });
            } else {
                if (globalChartActiveDisplayMode == chartDisplayModes.intraday) globalChartDom.series[0].update({ data: globalChartListingIntradayData[0] });
                else globalChartDom.series[0].update({ data: globalChartListingStockData[0] });
            }
            ...
            if (typeof ($('.IRChangeListing').html()) != "undefined") {
                $('.IRChangeListing select').bind('change', function () {
                    ...
                    if (typeof (menuTemplate_Lookup) == "function") {
                        changeListingLookup(globalRawClosePriceBundlePreload);
                        updateIRChangeListing();
                        attachClickHandlers('IRLookupModule');
                    }
                });
            ...
            if (typeof ($('.IRChangeListing').html()) != "undefined") {
                $('.IRChangeListing select').bind('change', function () {
                    ...
                    if (typeof (menuTemplate_Calc) == "function") {
                        changeListingCalc(globalRawClosePriceBundlePreload);
                        updateIRChangeListing();
                        attachClickHandlers('IRCalcModule');
                    }
                });
...
//Date.UTC(toDate.year(), toDate.month(), toDate.date(), toDate.hour(), toDate.minute())
    ...
    if (clientStyle.chart_settingsHighLow && $('.IRChartSettingsBody .basicButtonLook.highLow').hasClass('active')) {
        updateHighLowPlotLineToChartCustom(globalChartDom.yAxis[0].getExtremes());
    } else {
        removeHighLowPlotLineToChartCustom();
    }
...
function getLogoPath() {
    ...
    if (IRToolsNewsArticleHTMLModule) {
        ...
        logoPath += fetchedCustomerKey + "/logo.png?v=" + new Date().getTime();
    }
    else {
...
function updateTooltipComparisonDC(date) {
    ...
    try {
        ...
        var newDateIndex = getClosestDateIndexForListingClosePrice(date);
        tooltipStr += "<div class=\"IRChartTooltipDate\">" + new moment(globalChartListingStockDataOHLCV[globalActiveListingIndex][newDateIndex][0]).format(clientStyle.formatDate) + "</div>";
        tooltipStr += "<div><span class=\"subHeader\"><span class=\"colorIndicator active\" style=\"border-left-color: " + globalChartColours[0] + ";\"></span>" + globalRawStockData[globalActiveListingIndex].symbol + ": </span>" +
...
function applyAnalysisToChart(dataTA, TAShort, color, TAtype, period) {
    ...
    if (TAtype == 'dual') {
        ...
        percHeightMain = 100 - (15 * activeTAdual.length);

        var percHeightVolume = (clientStyle.chart_VolumeHeight * percHeightMain) / 100;
        percHeightVolume = percHeightMain < 50 ? percHeightMain : percHeightVolume;
        ...
        globalChartDom.yAxis[1].update({
            top: (percHeightMain - percHeightVolume) +'%',
            height: percHeightVolume + '%'
        });
