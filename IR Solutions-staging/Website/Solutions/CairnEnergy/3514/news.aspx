        ...
        <div>
			...
			<div class="IRNewsTool table-look horizontal responsive">
                ...
                <div class="IRDataGroup" id="{{storyID}}">
                    <div class="IRData IRDate column-first" id="{{showDateWithFormat timestamp 'YYYY-MMM-DD'}}">{{showDateTime timestamp}}</div>
...
<script type="text/javascript">
    ...
    function applyIRNewsPagination() {
        ...
        if (IRNewsPaginationApplied) {
            ...
            if (typeof ($('.IRNewsPagination').html()) != 'undefined') {
                ...
                var paginationTmp = "";
                paginationTmp += '<div class="IRNewsPaginationPageNew IRNewsPaginationPagePrev" style="display:inline-block;" id="prev"><</div>';
    ...
    function updateIRNewsPagination(page) {
        ...
        if (maxPage <= 5) {
            if (currentPage == 1) {
                $('.IRNewsPaginationPagePrev').css('background', '#b0ccd7');
            } else {
                $('.IRNewsPaginationPagePrev').css('background', '#6199af');
            }
        }
        ...
        if (minPage >= globalNewsPagesInTotal - 5) {
            ...
            if (currentPage == globalNewsPagesInTotal) {
                $('.IRNewsPaginationPageNext').css('background', '#b0ccd7');
            } else {
                $('.IRNewsPaginationPageNext').css('background', '#6199af');
            }
        }
                   ...
                   // console.log('IRNewsPaginationApplied: applied1');
                    ...
                    $('.IRNewsPaginationPageNew').on('click', function () {
                        ...
                        if ($(this).attr('disabled') != 'disabled') {
                            ...
                            if (IRNewsPaginationActivePage == 1) {
                                $('#prev').attr("disabled", true);
                            } else {
                                $('#prev').attr("disabled", false);
                                
                            }
                            if (IRNewsPaginationActivePage == globalNewsPagesInTotal) {
