...
function retrieveAndPostValues()
{
    ...
    emailAlertsManagerURL += 'IR/Tools/EmailAlertsWithPush/EmailAlertsManager.aspx?ts=' + (new Date).getTime(); // Accessible from asp/ir/*
...
function checkReleases(check)
{
    ...
    else if ($('#' + check + '').hasClass('checked')) {
        ret = 'checked';
    }
    return ret;
}
function checkInputFields()
{
    ...
    if (activePage == 1) {
        ...
        var ignoreCheckForReleaseA = false;
        if ($('#ignoreCheckForReleaseA').length == 1) {
            ignoreCheckForReleaseA = true;
        }
