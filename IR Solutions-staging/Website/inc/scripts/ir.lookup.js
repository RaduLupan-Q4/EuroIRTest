$(function ()
{
    ...
    function populateList(closePrices)
    {
        ...
        //    toDateRaw = getParameterByName("to"),
        //    frequency = getParameterByName("frequency") || "daily",
        //    fromDate = (fromDateRaw) ? new moment(fromDateRaw) : new moment(stockData[0].date),
        //    toDate = (toDateRaw) ? new moment(toDateRaw) : new moment(stockData[stockData.length - 1].date);


        //var a = new moment(stockData[0].date);
    ...
    function lookup()
    {
        ...
        if (readFormat().toLowerCase() === "excel") {
            ...
            closePriceRequest.done(function (closePrices)
            {
                ...
                "fileName":"String",
                "data":[{"date":"String","openPrice":0,"high":0,"low":0,"closePrice":0,"volume":0}],
                "apiVersion":1,
                "solutionID":10010,
            ...
            url += (url.indexOf("?") === -1) ? "?" : "&";
            url += "mode=list&from=" + readFromDateLookupResults() + "&to=" + readToDate() + "&frequency=" + readFrequency();
