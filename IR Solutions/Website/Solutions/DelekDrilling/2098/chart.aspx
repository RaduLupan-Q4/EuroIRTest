...
<script id="IRQuoteTableTemplate" type="text/x-handlebars-template">
    ...
    <table class="IRQuoteModule table-look horizontal responsive">
            <tr>
                ...
                <th class="Header change">{{headers/t_change_in_price}}</th>
                <th class="Header bid">{{headers/t_bid}}</th>
                <th class="Header ask">{{headers/t_ask}}</th>
                <th class="Header volume">{{headers/t_share_value}}</th>
            ...
            <tr>
                ...
                <td class="Data volume">{{shareVolume stocks/volume stocks/vwap}}</td>
                <td class="Data high">{{decimals stocks/high}}</td>
                <td class="Data low">{{decimals stocks/low}}</td>
                <td class="Data low">{{showLondonMarketCapM stocks/marketCap}}m</td>
...
<script>
    Handlebars.registerHelper('shareVolume', function (volume, vwap) {

        var number = Math.round((volume * vwap) / 100);

        var formattedNumber = formatLocal(number);
