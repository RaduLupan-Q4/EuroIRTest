                ...
                $.when(requestClosePriceListingData)
                    .done(function (data) {
                        ...
                        $('#HistoricalSharePriceButton').click(function () {
                            ...
                            var chosen_openPrice = globalClosePriceData[closestDate].openPrice;
                            $('.single-result-item-open').text('Open: ' + chosen_openPrice);
                            var chosen_high = globalClosePriceData[closestDate].high;
                            $('.single-result-item-high').text("Day's High: " + chosen_high);
                            var chosen_low = globalClosePriceData[closestDate].low;
                            $('.single-result-item-low').text("Day's Low: " + chosen_low);
                            var chosen_volume = globalClosePriceData[closestDate].volume;
                            $('.single-result-item-volume').text("Volume: " + chosen_volume);
                            var chosen_closePrice = globalClosePriceData[closestDate].closePrice;
                            $('.single-result-close-price-value').text(chosen_closePrice);
                            var currency = requestStockData.responseJSON.data[0].currency;
                            $('.single-result-close-price-currency').text(currency);
                            //       }
                            //   }
                        ...
                        function dataArrayOutputter(frequency, closePriceArray) {
                            ...
                            for (var i = 0; i < filteredDataLength; i++) {
                                ...
                                if (triggertimestamp != previousTriggertimestamp /*&& previousTriggertimestamp != startDay*/) {
                                    ...
                                    sharePriceArray.unshift(fullDate.closePrice);
