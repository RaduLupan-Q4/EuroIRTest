...
$.getJSON(getServiceEngingeURL() + 'RequestInsiderEuroclearData?apiversion=' + clientApiVersion + '&lcid=' + clientLCID + '&customerKey=' + clientCustomerKeyRequired + '&solutionID=' + clientSolutionID + '&instrumentid=' + IRInstrumentId, function (data) {
    allData = data;
...
function calculateMembersOfTheBoardTotals() {
    var value = [];
    var total = 0;
    $(".table-look.insiders .Data.shares").each(function () {
        var num = parseFloat($(this).text().replace(/(\d+).(?=\d{3}(\D|$))/g, "$1"));
        if (!isNaN(num)) {
            value.push(num);
        }
    });
    ...
    $(".table-look.insiders .total.shares").text(formatNumberWithLocalDelimiters(total));
    ...
    $(".table-look.insiders .total.k-share").text(formatNumberWithLocalDelimiters(total));
    ...
    $(".table-look.insiders .total.options").text(formatNumberWithLocalDelimiters(total));
};

function calculateManagementTotals() {
    var value = [];
    var total = 0;
    $(".table-look.management .Data.shares").each(function () {
        var num = parseFloat($(this).text().replace(/(\d+).(?=\d{3}(\D|$))/g, "$1"));
        if (!isNaN(num)) {
            value.push(num);
        }
    ...
    $(".table-look.management .total.shares").text(formatNumberWithLocalDelimiters(total));
    //____________k-share_________________//
    var value = [];
    var total = 0;
    ...
    $(".table-look.management .total.k-share").text(formatNumberWithLocalDelimiters(total));
    ////____________options_________________//
    var value = [];
    var total = 0;
    ...
    $(".table-look.management .total.options").text(formatNumberWithLocalDelimiters(total));
};
function calcculateAuditorsTotals() {
    var value = [];
    var total = 0;
    $(".table-look.auditors .Data.shares").each(function () {
        var num = parseFloat($(this).text().replace(/(\d+).(?=\d{3}(\D|$))/g, "$1"));
        if (!isNaN(num)) {
            value.push(num);
        }
    ...
    $(".table-look.auditors .total.shares").text(formatNumberWithLocalDelimiters(total));

    ////____________options_________________//
    var value = [];
    var total = 0;
    $(".table-look.auditors .Data.options").each(function () {
        var num = parseFloat($(this).text().replace(/(\d+).(?=\d{3}(\D|$))/g, "$1"));
        if (!isNaN(num)) {
            value.push(num);
        }
    ...
    $(".table-look.auditors .total.options").text(formatNumberWithLocalDelimiters(total));
}

function calculateSummary() {
    //Total shares
    var auditorsShareTotal = parseFloat($(".table-look.auditors .total.shares").text().replace(',', ''));
    var managementShareTotal = parseFloat($(".table-look.management .total.shares").text().replace(',', ''));
    var memberOfTheBoardTotal = parseFloat($(".table-look.insiders .total.shares").text().replace(',', ''));
    var totalShareSummary = (auditorsShareTotal + managementShareTotal + memberOfTheBoardTotal);
    $(".table-look.totalSummary .total.shares").text(formatNumberWithLocalDelimiters(totalShareSummary));

    //Total Options
    var auditorsOptionTotal = parseFloat($(".table-look.auditors .total.options").text().replace(',', ''));
    var managementOptionTotal = parseFloat($(".table-look.management .total.options").text().replace(',', ''));
    var memberOfTheOptionTotal = parseFloat($(".table-look.insiders .total.options").text().replace(',', ''));

    var totalOptionSummary = (auditorsOptionTotal + managementOptionTotal + memberOfTheOptionTotal);
    $(".table-look.totalSummary .total.options").text(formatNumberWithLocalDelimiters(totalOptionSummary));
...
Handlebars.registerHelper('if_eq', function (a, b, opts) {
    ...
    if (b == "membersOfTheBoard") {
        ...
        if (jQuery.inArray(a, membersPositionID) !== -1) {
            return opts.fn(this);
        } else {
    ...
    //management
    if (b == "management") {
        var membersPositionID = [122, 101, 190];
...
Handlebars.registerHelper('getShare', function (holdings, opts) {
    ...
    for (var i = 0; i < holdings.length; i++) {
        if (holdings[i].name == opts && holdings[i].hasOwnProperty('lastUpdate')) {
            
            if (holdings[i].amount != 0) {
                result += holdings[i].amount;
            }           
        }
    ...
    return formatNumberWithLocalDelimiters(result);
});

Handlebars.registerHelper('class', function (opts) {
    if (opts % 2 == 0) {
        return "rowHigh";
    } else {
        return "rowLow";
...
Handlebars.registerHelper('getSharesOfInterest', function (holdings, opts) {
    var result = 0;
    for (var i = 0; i < holdings.length; i++) {
        if (holdings[i].name == opts && !holdings[i].hasOwnProperty('lastUpdate')) {
            result = holdings[i].amount;
    ...
    return formatNumberWithLocalDelimiters(result) + " [0]";
});
...
Handlebars.registerHelper('getOptions', function (holdings, opts) {
    var result = 0;
