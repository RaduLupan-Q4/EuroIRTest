...
function populateList(closePrices) {
    ...
    //fromDate = (fromDateRaw) ? new Date(fromDateRaw) : new Date(stockData[globalActiveListingIndex].date),
...
function stockDataToChartData(stockData, _dateProperty, _priceProperty) {
    ...
    for (var i = 0; i < stockData.length; i++) {
        ...
        //transformedData.push([new moment(stockData[i][dateProperty]).valueOf(), stockData[i][priceProperty]]);
...
function ajax_download(url, data) {
    ...
    if (clientCustomerKeyRequired.toLowerCase() == 'pushpay' && /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream) {
        ...
        //$('.ExcelDownladForm').html(form);
        //$('.ExcelDownladForm').find('form').submit();
    ...
    if (readFormat().toLowerCase() === "excel") {
        ...
        requestClosePriceListingData.done(function (closePrices) {
            if (IRCustomModule) {
                ...
                var downscaledData = getCroppedDownscaledData(globalClosePriceData, startDate, endDate, frequency);
                ...
                var downscaledData = getCroppedDownscaledData(closePrices.data[globalActiveListingIndex].data, startDate, endDate, frequency);
                var stringified = JSV.stringify(downscaledData); // Using JSV because service engine expects that.
                var tableHeader = {
                    t_date: translations.t_date,
                    t_open: translations.t_open,
        ...
        //url = url.replace('listing=0', 'listing=' + globalActiveListingIndex);
...
function initializeLookup(data) {
    ...
    if (getParameterByName("mode") === "list") {
        ...
        var compiledIRLookupTemplate = Handlebars.compile(lookupTemplate);
    ...
    $("#from-datepicker").datepicker({ showOn: "button", buttonImage: getImagePath() + "icons/calendar.png", buttonImageOnly: true, onSelect: datePickerChangeHandler, defaultDate: fromDateM, maxDate: new Date() });
    $("#to-datepicker").datepicker({ showOn: "button", buttonImage: getImagePath() + "icons/calendar.png", buttonImageOnly: true, onSelect: datePickerChangeHandler, defaultDate: toDateM, maxDate: new Date() });
...
function populateChart(closePriceListingData) {
    ...
    transformedData = stockDataToChartData(closePriceListingData.data[globalActiveListingIndex].data);
